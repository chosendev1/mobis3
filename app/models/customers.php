...
class Customers extends CI_Controller {
	...
	$dataArray =  array("branch_no" => $this->input->post('branch_no'),
		...
		$new_cust = @mysql_query("INSERT INTO member (group_id,branch_no, first_name, last_name, telno, email, address, kin, kin_telno, dob, sex,branch_id) VALUES ('".$group_id."',$branch_id, '".$first_name."', '".$last_name."', '".$phone."', '".$email."', '".$paddress."', '".$kin_name."', '".$kin_phone."', '".$dob."', '".$sex."',".$branch_id.")");
		$action = "INSERT INTO member (group_id,branch_no, first_name, last_name, telno, email, address, kin, kin_telno, dob, sex) VALUES ('".$group_id."','".$branch_id."', '".$first_name."', '".$last_name."', '".$phone."', '".$email."', '".$paddress."', '".$kin_name."', '".$kin_phone."', '".$dob."', '".$sex."', ".$branch_id.")";
		...
		$last_res = @mysql_query("select max(mem_no) as mem_no from member where group_id =".$group_id." and branch_id='".$branch_id."' and non_individual= 1");
		...
		array_push($arr,preg_replace("/^".$group_no."/", "", $grpNo));
		}
		}
		$last_memNo=max($arr);
		$response ='';
		...
		if(empty($group_id))
			...
			$last_no = preg_replace("/^".$branch['prefix'].$group_no."/", "", $last_memNo);
		...
		$mem_no = $branch['prefix'].$group_no.str_pad($mem_no,2, "0", STR_PAD_LEFT);
	...
	//var_dump($dataArray);
		...
		$response .= "Customer successfully Added<br>";
		...
		if($_FILES['fname']['name'] != NULL){
			...
			if(!move_uploaded_file($_FILES['fname']['tmp_name'], "uploads/customerfiles/".$fname_name))
				...
				for ($i = 2; $i<=$data->sheets[0]['numRows']; ++$i) 
				{
					...
					$dbi['grpNo'][$i] = $data->sheets[0]['cells'][$i][10]=='' ? 0 : $data->sheets[0]['cells'][$i][10];
					
					        $grpNo = $data->sheets[0]['cells'][$i][10];
					if($grpNo > 0)        
