...
$curlink = $this->uri->segment(1);
       ...
       $s = mysql_fetch_array(mysql_query("select u.cashAccountId,u.userId,b.account_balance,b.id from users u join bank_account b on(u.cashAccountId = b.id) where u.userId='".CAP_Session::get('userId')."'")); 
        CAP_session::set("account_assigned",$s['cashAccountId']);
        $balance=libinc::cashAccountBalance($s['cashAccountId'])
