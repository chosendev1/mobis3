...
function ratios($year, $month, $mday, $format){
	...
	$content = "
	<table height=200 border='1' cellpadding='0' cellspacing='1' style='border-collapse: collapse' bordercolor='#111111' width='50%' id='AutoNumber2' align=center> ";
	if($year == ''){
...
//OPERATING EXPENSES
	...
	for($i=0; $i <3; $i++){
		...
		//LIQUID INVESTMENTS
		$invest_res = mysql_query("select sum(i.amount) as amount from investments i join accounts a on i.account_id=a.id where i.date < '".$apparent_date."' and i.account_id not in (select account_id from sold_invest) and a.account_no like '113%'");
		$invest = mysql_fetch_array($invest_res);
		$invest_amt = ($invest['amount'] != NULL) ? $invest['amount'] : 0;
...
//reports_header();
ratios($_GET['year'], $_GET['month'], $_GET['mday'], $_GET['format']);
//reports_footer();
?>
