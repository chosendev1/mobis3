...
class TransactionAccounts extends CI_Controller {
   ...
   public function importLoanPayments(){
    	...
    	$response = "Upload just failed";
	$this->data['response'] = $response;
	$this->data['links'] = array("configuration/postLoans" => "Return to importing Repayments");
	...
	if($_FILES['fname']['name'] != NULL){
		...
		if(!move_uploaded_file($_FILES['fname']['tmp_name'], "uploads/transactionfiles/".$fname_name)){
			...
			return false;
				...
				if($cash_account_id=='' || $contact=='' || $file_ref==''){
					$this->data['response'] = "Bulk Posting rejected! Fill in all the fields!";
					$this->load->view("response/response", $this->data);
					$this->load->view("includes/footer",  $this->data);
					return false;
				...
				for($j = 2;  $j <= $data->sheets[0]['numRows']; $j++){
					...
					if($mem_no == ''){
						$this->data['response'] = "Import rolled back: Member No in line ".$j."nis Empty";
						rollback();
						$this->load->view("response/response", $this->data);
						$this->load->view("includes/footer",  $this->data);
						return false;
					...
					$loan_res =mysql_query("select id from disbursed where applic_id=$loan_no");
					if(mysql_numrows($loan_res) == 0){
			...
			if(! mysql_query("insert into payment set disbursement_id='".$dis_id."',amount=$amount, receipt_no='".$receipt_no."', princ_amt='".$principal."', int_amt='".$interest."', date='".$date."', bank_account='$cash_account_id', branch_id='".$branch_id."',file_ref='".$file_ref."',contact='".$contact."'")){	
			...
			$action = "insert into payment set loan_id='".$loan_no."', receipt_no='".$receipt_no."', princ_amt='".$principal."', int_amt='".$interest."', date='".$date."', bank_account='$cash_account_id', branch_id='".$branch_id."'";
			$msg = "Imported a Repayment:".($principal+$interest)." of which principal=".$principal." and interest=".$interest." for customer: IPPS ".$accno['ipps']." ".$accno['first_name']." ".$accno['last_name']." - ".$accno['mem_no']; 
			log_action($_SESSION['user_id'],$action,$msg);
