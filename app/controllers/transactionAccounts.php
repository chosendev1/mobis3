...
class TransactionAccounts extends CI_Controller {
	...
	*/
	...
	$this->data['links'] = array("transactionAccounts/addBulkPost" => "Return to import transactions",
    	...
    	$vdate = explode('-', $date);
    	 ...
    	 $this->data['response'] = "Please Enter a Valid Date!!!";
	...
	if($_FILES['fname']['name'] != NULL){
		...
		log_action($_SESSION['user_id'],$action,$msg);
							...
							if(! mysql_query("insert into other_income set amount=1000, date='".$date."', account_id=448, file_ref='".$file_ref."',branch_id='".$branch_id."', contact='".$contact."', mode='".$memAcctId."',bank_account='".$bank_acct_id."',transaction='Monthly Subscription'")){
		...
		if(!move_uploaded_file($_FILES['fname']['tmp_name'], "uploads/transactionfiles/".$fname_name)){
			...
			return false;
				...
				for ($i = 2;  $i <= $data->sheets[0]['numRows']; ++$i){
					...
					$mem_id = libinc::getItemById("member", $mem_no,"mem_no", "id");
					$branch_id = libinc::getItemById("member", $mem_no,"branch_id", "id");
					$memaccount_id= libinc::getItemById("mem_accounts", $mem_id,"mem_id", "id");
  ...
  public function importLoanPaymentsWithLoanNumbers(){
    	$this->load->view("includes/headerForm", $this->data);
    	ini_set('max_execution_time',600);
    	$response = "Upload just failed";
	$this->data['response'] = $response;
		...
		if(!move_uploaded_file($_FILES['fname']['tmp_name'], "uploads/transactionfiles/".$fname_name)){
			...
			return false;
				...
				for($j = 2; $j <= $data->sheets[0]['numRows']; $j++){
					...
					if($loan_no == ''){
						$this->data['response'] = "Import rolled back: Loan No in line ".$j." is Empty";
						//rollback();
						$this->load->view("response/response", $this->data);
						$this->load->view("includes/footer",  $this->data);
						return false;
					...
					$applic =mysql_query("select id from loan_applic where id=$loan_no");
		...
		*/		
		...
		$total_int_paid=$int_paid['int_paid'];
			...
			$interest_bal=$total_int-$total_int_paid;
		        ...
		        if($interest_bal <= 0)	        		        
		        $interest_excess3=$interest_excess2;
		        }
		        
		        if($principal_excess2 > 0){
		        if($principal_bal <= 0)	        		        
		        $principal_excess3=$principal_excess2;
		        }
			...
			commit();
                        ...
                        //$this->data['response'] = "One of the loans for customer (".$mem_no.") in line ".$j." not disbursed Yet!";
                        $this->data['response'] = "Loan for customer (".$mem_no.") in line ".$j." does not exist!";
						$this->load->view("response/response", $this->data);
						$this->load->view("includes/footer",  $this->data);
						return false;    
                   }			
...
public function importLoanPayments(){
	...
	if($_FILES['fname']['name'] != NULL){
		...
		$total_princ_paid=$princ_paid['princ_paid'];
		$int_paid=mysql_fetch_array(mysql_query("select sum(int_amt) as int_paid from payment where disbursement_id=$dis_id"));
		$total_int_paid1=$int_paid['int_paid'];
		$int_waived=mysql_fetch_array(mysql_query("select sum(waived_interest_amt) as int_waived from waived_loans where disbursement_id=$dis_id"));
		$total_int_waived=$int_waived['int_waived'];
		$total_int_paid	= $total_int_paid1+$total_int_waived;	
				               //get total principal and interest required	
			$princ_res=mysql_query("select sum(princ_amt) as principal from schedule where approval_id=$approval_id");
			$princ = mysql_fetch_array($princ_res);
			$total_princ=$princ['principal'];
