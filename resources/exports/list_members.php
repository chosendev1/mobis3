...
//LIST CUSTOMERS
function membersList($type, $mem_no, $mem_name, $format,$branch_id,$num_rows,$stat)
{
	...
	if (strtolower($type) == 'all'){
		$mem = @mysql_query("select id, mem_no, first_name, last_name, sex, (DATEDIFF(NOW(), dob)/365) as age, kin, kin_telno, address, status,  email, telno from member  where (first_name like '%".$mem_name1."%' or last_name  like '%".$mem_name1."%') and mem_no like '%".$mem_no1."%' ".$branch." order by first_name, last_name asc limit ".$stat.",".$num_rows);
		$head = "ALL CUSTOMERS LIST";
	}elseif (strtolower($type) == 'members'){
		$mem = @mysql_query("select id, mem_no, first_name, last_name, sex, (DATEDIFF(NOW(), dob)/365) as age, kin, kin_telno, address, status,  email, telno  from member where id in (select mem_id from shares) and (first_name like '%".$mem_name1."%' or last_name  like '%".$mem_name1."%') and mem_no like '%".$mem_no1."%'  ".$branch." order by first_name, last_name asc limit ".$stat.",".$num_rows);
...
//reports_header();
$type;
//membersList($_GET['type'], $_GET['mem_no'], $_GET['mem_name'], $_GET['format'],$_GET['branch_id'],$_GET['num_rows'],$_GET['stat']);
//reports_footer();
?>
