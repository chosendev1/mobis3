...
$content .= '<div class="panel-body pt0 pb0">
	...
	$content .= '<div class="form-group">
                                            ...
                                            <div class="col-sm-6"><input type=text name="hname" id="hname" class="form-control"></div>                                            
                                            </div>'; 
			$content .= '<div class="form-group">
                                            <label class="col-sm-3 control-label">Date:</label>
                                            <div class="col-sm-6"><input type=text name="date" id="date" class="form-control"></div>                                            
                                            </div>'; 
...
{	
	...
	{
		$content .= "<font color=red>Holiday '$hname' successfully registered.</font><br>";
...
function listHolidays()
{
	...
	if (@mysql_num_rows($res) > 0)
	{
		...
		$content .= "<thead><th>#</th><th>NAME OF HOLIDAY</th><th>DATE</th><th></th></thead><tbody>";
...
$content .= '<div class="panel-body pt0 pb0">
	...
	$content .= '<div class="form-group">
                                            <label class="col-sm-3 control-label">Name of Holiday:</label>
                                            <div class="col-sm-6"><input type=text name="hname" id="hname" value="'.$row['name'].'" class="form-control"></div>                                            
                                            </div>'; 
			$content .= '<div class="form-group">
                                            <label class="col-sm-3 control-label">Date:</label>
                                            <div class="col-sm-6"><input type=text name="date" id="date" value="'.$row['year'].'-'.$row['month'].'-'.$row['mday'].'" class="form-control"></div>                                            
                                            </div>'; 			
...
function update2Holiday($id, $name, $date){
	...
	if(! mysql_query("update holiday set name='".$name."', date='".$date."' where id='".$id."'"))
		...
		$resp->assign("status", "innerHTML", "<FONT COLOR=RED>Holiday updated successfully</font>");
	}
	return $resp;
}
