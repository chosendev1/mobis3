...
function list_disbursed($cust_name, $cust_no, $account_name, $loan_officer, $from_date,$to_date,$branch_id){
	...
	$sth = mysql_query("select d.id as id, applic.id as applic_id, m.first_name as first_name, m.last_name as last_name, m.mem_no as mem_no, o.first_name as of_first_name, o.last_name as of_last_name, a.account_no as account_no, a.name as account_name, d.id as id, d.amount as amount, d.cheque_no as cheque_no, d.date as date, d.balance as balance, d.period as loan_period, d.int_rate as int_rate, d.int_method as int_method, d.grace_period as grace_period, d.writeoff_period as writeoff_period, d.arrears_period as arrears_period, d.written_off as written_off, d.bank_account as bank_account from disbursed d join loan_applic applic on d.applic_id=applic.id join member m on applic.mem_id=m.id join loan_product p on applic.product_id=p.id join accounts a on p.account_id= a.id join officer o on applic.officer_id=o.id where (m.first_name like '%".$name."%' or m.last_name like '%".$name."%') and m.mem_no like '%".$mem_no."%' and a.name like '%".$account_name."%' and ".$officer." and d.date >= '".$from_date."' and d.date <='".$to_date."' ".$branch_." order by o.first_name, o.last_name, d.cheque_no, m.first_name, m.last_name");
...
function risk_ageing($cust_name, $cust_no, $account_name, $loan_officer, $from_date,$to_date,$branch_id){
	...
	////$max_page = ceil(mysql_num_rows($sth)/$num_rows);
	$sth2 = mysql_query("select d.id as id, applic.id as applic_id, m.first_name as first_name, m.last_name as last_name, m.mem_no as mem_no, o.first_name as of_first_name, o.last_name as of_last_name, a.account_no as account_no, a.name as account_name, d.id as id, d.amount as amount, d.date as date, d.balance as balance, d.period as loan_period, d.int_rate as int_rate, d.int_method as int_method, d.grace_period as grace_period, d.writeoff_period as writeoff_period, (DATEDIFF(CURDATE(), d.last_pay_date)/30) as since_period, d.arrears_period as arrears_period, d.written_off as written_off, d.bank_account as bank_account, d.last_pay_date as last_pay_date, d.pay_freq as pay_freq from disbursed d join loan_applic applic on d.applic_id=applic.id join member m on applic.mem_id=m.id join loan_product p on applic.product_id=p.id join accounts a on p.account_id= a.id join officer o on applic.officer_id=o.id where (m.first_name like '%".$name."%' or m.last_name like '%".$name."%') and m.mem_no like '%".$mem_no."%' and a.name like '%".$account_name."%' and ".$officer." and d.date >= '".$from_date."' and d.date <='".$to_date."' and d.written_off='0' ".$branch_." order by o.first_name, o.last_name, m.first_name, m.last_name ");
	if(@ mysql_numrows($sth) == 0){
		$cont = "<font color=red>No disbursed loans in your search options</font>";
		$resp->assign("status", "innerHTML", $cont);
		return $resp;
...
function list_outstanding($cust_name, $cust_no, $account_name, $loan_officer, $from_date,$to_date,$branch_id){
	...
	$total_amount = mysql_fetch_assoc(mysql_query($sth = "select sum(d.amount) as disbursed, sum(d.balance) as balance from disbursed d join loan_applic applic on d.applic_id=applic.id join member m on applic.mem_id=m.id join loan_product p on applic.product_id=p.id join accounts a on p.account_id= a.id join officer o on applic.officer_id=o.id where (m.first_name like '%".$name."%' or m.last_name like '%".$name."%') and m.mem_no like '%".$mem_no."%' and a.name like '%".$account_name."%' and ".$officer." and d.date >= '".$from_date."' and d.date <='".$to_date."' and d.balance >0  and d.written_off=0 ".$branch_." order by o.first_name, o.last_name, d.cheque_no, m.first_name, m.last_name"));
	$former_officer ="";
	$i=$stat+1;
	$amt_sub_total=0;
	$bal_sub_total=0;
...
function list_due($cust_name, $cust_no, $account_name, $loan_officer, $date,$branch_id){
	...
	//$total_amount = mysql_fetch_assoc(mysql_query($sth = "select sum(d.amount) as disbursed, sum(d.balance) as balance from disbursed d join loan_applic applic on d.applic_id=applic.id join member m on applic.mem_id=m.id join loan_product p on applic.product_id=p.id join accounts a on p.account_id= a.id join officer o on applic.officer_id=o.id where (m.first_name like '%".$name."%' or m.last_name like '%".$name."%') and m.mem_no like '%".$mem_no."%' and a.name like '%".$account_name."%' and ".$officer." and d.date <='".$to_date."' and d.balance >0 and d.cheque_no like '%".$cheque_no."%' and d.written_off=0 ".$branch." order by o.first_name, o.last_name, d.cheque_no, m.first_name, m.last_name"));
	$former_officer ="";
	$i=$stat+1;
	$amt_sub_total=0;
	$bal_sub_total=0;
