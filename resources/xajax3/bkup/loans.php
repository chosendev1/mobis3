...
$content .= '<div class="panel-body pt0 pb0">
	...
	while($level1 = mysql_fetch_array($level1_res)){
		...
		if( mysql_numrows($level2_res) == 0){
			$resp->AddAlert($level1['account_no']);
...
function edit_loanproduct($loanproduct_id){
	...
	$content = "<center><font color=#00008b size=3pt><b>EDIT LOAN PRODUCT</b></font></center><table height=100 border='0' cellpadding='0' cellspacing='0' style='border-collapse: collapse' bordercolor='#111111' width='70%' id='AutoNumber2' align=center>
	<tr bgcolor=lightgrey><td>Product Name</td><td><select name='account_id' id='account_id'><option value='".$former['account_id']."'>".$former['account_no']." - ".$former['name'];
	$level1_res = mysql_query("select * from accounts where account_no like '111%' and account_no not like '111' and account_no <= 1119 and branch_no like '".$_SESSION['branch_no']."' and id not in (select p.account_id from loan_product p join accounts a on p.account_id=a.id where a.branch_no like '".$_SESSION['branch_no']."')");
	...
	$content .= "
	<tr bgcolor=white><td>Interest Rate (% per Annum)</td><td><input type=numeric(6,3) id='int_rate' value='".$former['int_rate']."'></td></tr>
	<tr bgcolor=lightgrey><td>Penalty Rate (% per Annum)</td><td><input type=numeric(6,3) id='penalty_rate' value='".$former['penalty_rate']."'></td></tr>
	<tr bgcolor=white><td>Interest Method</td><td><select id='int_method'><option value='".$former['int_method']."'>".$former['int_method']."<option value='Declining Balance'>Declining Balance<option value='Flat'>Flat<option value='Armotised'>Armotised</select></td></tr>
	<tr bgcolor=lightgrey><td>Grace Period (Months)</td><td><input type=int id='grace_period' value='".$grace_period."'></td></tr>
	<tr bgcolor=white><td>Arrears Maturity (Months before a payment becomes arrears)</td><td><input type=int id='arrears_period' value='".$arrears_period."'></td></tr>
	<tr bgcolor=lightgrey><td>Loan Period</td><td><input type=int id='loan_period' value='".$loan_period."'></td></tr>
	<tr bgcolor=white><td>Maximum Loan Amount</td><td><input type=bigint id='max_loan_amt' value='".$former['max_loan_amt']."'></td></tr>
	<tr bgcolor=lightgrey><td>Write-off Period (Months)</td><td><input type=int id='writeoff_period' value='".$writeoff_period."'></td></tr>
	<tr bgcolor=lightgrey><td></td><td><a onclick=\"xajax_delete_loanproduct('".$loanproduct_id."');\"><img src='images/btn_delete.jpg'></a>&nbsp;&nbsp;<a onclick=\"xajax_update_loanproduct('".$loanproduct_id."', getElementById('account_id').value, getElementById('int_rate').value, getElementById('penalty_rate').value, getElementById('int_method').value, getElementById('grace_period').value, getElementById('arrears_period').value, getElementById('loan_period').value, getElementById('max_loan_amt').value, getElementById('writeoff_period').value);\"><img src='images/btn_save.jpg'></a></td></tr>";
	$content .= "</table>";
	$resp->assign("display_div", "innerHTML", $content);
	return $resp;
}
...
function list_applics($cust_name, $cust_no, $account_name, $from_date,$to_date, $category,$branch_id){
	...
	$content .='</select>                                           
		 ...
		 $content .= "<div class='panel-footer'>                              
                                ...
                                <input type='button' class='btn  btn-primary' value='Search' onclick=\"xajax_list_applics(getElementById('cust_name').value, getElementById('cust_no').value, getElementById('account_name').value,getElementById('from_date').value, getElementById('to_date').value, '".$category."', getElementById('branch_id').value);\">
                            </div></div>
                        </form>
                        <!--/ Form default layout -->
                    </div></div>";
	...
	$sth = mysql_query("select count(applic.id) as num from loan_applic applic join member m on applic.mem_id=m.id join loan_product p on applic.product_id=p.id join accounts a on p.account_id=a.id join officer o on applic.officer_id=o.id left join disbursed dis on applic.id=dis.applic_id where (m.first_name like '%".$name."%' or m.last_name like '%".$name."%') ".$branch." and m.mem_no like '%".$mem_no."%' and a.name like '%".$account_name."%' and ".$choose." applic.date >='".$from_date."' and applic.date <= '".$to_date."' order by o.first_name, o.last_name, m.first_name, m.last_name, m.mem_no");
	
	//$resp->assign("status", "innerHTML", "select dis.id as disbursed_id, applic.group_id as group_id, applic.approved as approved, applic.reason as reason, (DATEDIFF(applic.date, m.dob)/365) as age, o.first_name as of_first_name, o.last_name as of_last_name, m.id as mem_id, m.sign_name as sign_name, m.photo_name as photo_name, m.mem_no as mem_no, m.first_name as first_name, m.last_name as last_name, applic.amount as amount, applic.date as date, applic.guarantor1 as guarantor1, applic.guarantor2 as guarantor2, applic.id as applic_id, a.name as account_name, a.account_no as account_no, m.id as mem_id, p.based_on as based_on from loan_applic applic join member m on applic.mem_id=m.id join loan_product p on applic.product_id=p.id join accounts a on p.account_id=a.id join officer o on applic.officer_id=o.id left join disbursed dis on applic.id=dis.applic_id where (m.first_name like '%".$name."%' or m.last_name like '%".$name."%') ".$branch." and m.mem_no like '%".$mem_no."%' and a.name like '%".$account_name."%' and ".$choose." applic.date >='".$from_date."' and applic.date <= '".$to_date."' order by o.first_name, o.last_name, m.first_name, m.last_name, m.mem_no");
	if(mysql_numrows($sth) == 0){
	...
	$sth2 = mysql_query("select dis.id as disbursed_id, applic.group_id as group_id, applic.approved as approved, applic.reason as reason, (DATEDIFF(applic.date, m.dob)/365) as age, o.first_name as of_first_name, o.last_name as of_last_name, m.id as mem_id, m.sign_name as sign_name, m.photo_name as photo_name, m.mem_no as mem_no, m.first_name as first_name, m.last_name as last_name, applic.amount as amount, applic.date as date, applic.guarantor1 as guarantor1, applic.guarantor2 as guarantor2, applic.id as applic_id, a.name as account_name, a.account_no as account_no, p.based_on as based_on from loan_applic applic join member m on applic.mem_id=m.id join loan_product p on applic.product_id=p.id join accounts a on p.account_id=a.id join officer o on applic.officer_id=o.id left join disbursed dis on applic.id=dis.applic_id where (m.first_name like '%".$name."%' or m.last_name like '%".$name."%') ".$branch." and m.mem_no like '%".$mem_no."%' and a.name like '%".$account_name."%' and ".$choose." applic.date >='".$from_date."' and applic.date <= '".$to_date."' order by o.first_name, o.last_name, m.first_name, m.last_name, m.mem_no");
	if(mysql_numrows($sth2) > 0){
		
	$former_officer ="";
	$i=$stat+1; 
	$amt_sub_total = 0;
	$total__amount = mysql_fetch_assoc(mysql_query("select sum(applic.amount) as amountd from loan_applic applic join member m on applic.mem_id=m.id join loan_product p on applic.product_id=p.id join accounts a on p.account_id=a.id join officer o on applic.officer_id=o.id left join disbursed dis on applic.id=dis.applic_id where (m.first_name like '%".$name."%' or m.last_name like '%".$name."%') ".$branch." and m.mem_no like '%".$mem_no."%' and a.name like '%".$account_name."%' and ".$choose." applic.date >='".$from_date."' and applic.date <= '".$to_date."' order by o.first_name, o.last_name, m.first_name, m.last_name, m.mem_no"));
...
//start time	
	...
	while($row = mysql_fetch_array($sth2)){
		...
		if($row['disbursed_id'] == NULL && $row['approved'] ==1)
			$disburse="<a href='javascript:;' onclick=\"xajax_add_disbursed('".$row['applic_id']."', '".$row['amount']."', '".$possible_amt."', '".$row['based_on']."','".$branch_id."');\">Disburse</a>";
		else
			$disburse = "--";
		//$color = ($i%2 == 0) ? "lightgrey" : "white";
...
function edit_applic($applic_id, $cust_name, $cust_no, $account_name, $from_date,$to_date, $category, $branch_id){
	$resp = new xajaxResponse();
	$former_res = mysql_query("select g.name as group_name, m.id as mem_id, m.first_name as first_name, m.last_name as last_name, m.mem_no as mem_no, ap.marital_status as status, ap.residence as residence, ap.telno as telno, ap.postal_address as postal_address, ap.amount as amount, date_format(ap.date, '%Y') as year, date_format(ap.date, '%m') as month, date_format(ap.date, '%d') as mday, ap.income1 as income1, o.id as officer_id, o.first_name as of_first_name, o.last_name as of_last_name, ap.income2 as income2, ap.reason as reason, ap.guarantor1 as guarantor1, ap.guarantor2 as guarantor2, ap.product_id as product_id, a.account_no as account_no, a.name as account_name from loan_applic ap join member m on ap.mem_id=m.id join loan_product p on ap.product_id=p.id join accounts a on p.account_id=a.id join officer o on ap.officer_id=o.id left join loan_group g on ap.group_id=g.id where ap.id='".$applic_id."'");
	...
	$content .= '</select></div>';
        ...
        // $content .= '<div class="form-group">
	                                $content .= '<div class="col-sm-6">
                                           ...
                                           <select id="applic_officer_id" class="form-control"><option value="'.$former['officer_id'].'">'.$former['of_first_name'].' '.$former['of_last_name'];
	$sth = mysql_query("select * from officer where position='Loan Officer' and id<>'".$former['officer_id']."'");
	while($row = mysql_fetch_array($sth)){
...
function insert_disbursed($applic_id, $product_id, $amount, $cheque_no, $int_rate, $penalty_rate, $int_method,  $grace_period, $writeoff_period, $arrears_period, $loan_period, $date, $based_on, $bank_account_id, $automatic, $disburse_method,$branch_id, $pay_freq, $repay_date){
	...
	//	$based_on = $prod['based_on'];
		...
		if($row['account_balance'] - $amount < $row['min_balance']){
			$resp->alert("Disbusement rejected! \n The disbursement would go below the minimum balance");
			return $resp;
		}
		*/
		$date= $calc->dateFormat($date, '%Y-%m-%d');
		$date = $date." ".date('H:i:s');
		start_trans();
		...
		$resp->assign("status", "innerHTML", "<font color=red>Loan disbursed</font>");
		...
		if(@ mysql_numrows($sth) > 0){
			$resp->assign("status", "innerHTML", "<font color=red>Disbursement done!</font>   <a href='list_schedule.php?loan_id=".$loan['last_id']."&applic_id=".$applic_id."&cust_name=".$cust_name."&cust_no=".$cust_no."&account_name=".$account_name."&loan_officer=".$loan_officer."&from_year=".$from_year."&from_month=".$from_month."&from_mday=".$from_mday."&to_year=".$to_year."&to_month=".$to_month."&to_mday=".$to_mday."&status=".$status."&format=excel' target=blank()><b>Print Repayment Schedule</b></a>");
		}else
 ...
 <p><h5>AMOUNT:&nbsp;<font color="#00BFFF">'.$former['amount'].'</font></h5></p>                                                                                                                                                                                                                                  
	             ...
	             $content .= '<div class="form-group">
                     ...
                     $content .= '<div class="form-group">
	                                   ...
	                                   <div class="col-sm-6">
                                           ...
                                           <input type=int value="'.($former['arrears_period']/30).'" id="arrears_period"></div>
	 				<div class="col-sm-6">
                                            <label class="control-label">Write-off Period(Months):</label>
                     ...
                     $content .= '<div class="form-group">
	                                   ...
	                                   <div class="col-sm-6">
                                            <label class="control-label">Loan Period(Months):</label>
                                          <input type=int value="'.($former['loan_period']/30).'" id="loan_period">
                                            </div>
                                            
                                           <div class="col-sm-6">
                                            <label class="control-label">Re-Payment Frequency:</label>
                                            <select id="pay_freq"><option value="30">Monthly<option value="1">Daily<option value="7">Weekly<option value="15">2 Weeks<option value="60">2 Months<option value="90">Quarterly<option value="120">4 Months<option value="150">5 Months<option value="180">6 Months<option value="210">7 Months<option value="240"> 8 Months<option value="270">9 Months<option value="300">10 Months<option value="330">11 Months<option value="360">Annually</select>
                                            </div>
                                            </div>';
	...
	$content .='</select></div></div>';
	 			...
	 			<div class="col-sm-6">
                      ...
                      $content .= "<div class='panel-footer'><button class='btn btn-default' onclick=\"xajax_list_disbursed('".$cust_name."', '".$cust_no."', '".$account_name."', '".$loan_officer."', '".$from_date."','".$to_date."', '".$cheque_no."','".$branch_id."')\">Back</button>&nbsp;<button class='btn btn-default' onclick=\"xajax_delete_disbursed('".$id."', '".$former['amount']."', '".$former['bank_account']."','".$former['branch_id']."');\">Delete</button>&nbsp;<button class='btn btn-primary' onclick=\"xajax_update_disbursed('".$id."', '".$applic_id."', '".$former['amount']."', getElementById('cheque_no').value, getElementById('int_rate').value, getElementById('penalty_rate').value, getElementById('int_method').value,  getElementById('grace_period').value,  getElementById('writeoff_period').value, getElementById('arrears_period').value, getElementById('loan_period').value, getElementById('automatic').value, getElementById('year').value,  getElementById('month').value,  getElementById('mday').value, getElementById('pay_freq').value, getElementById('repay_date').value);\">Update</button>";
...
function update2_disbursed($id, $applic_id, $amount, $cheque_no, $int_rate, $penalty_rate, $int_method,  $grace_period,  $writeoff_period, $arrears_period, $loan_period, $automatic,  $date, $pay_freq, $repay_date){
	...
	if(@ mysql_numrows($sth) > 0){
		$resp->assign("status", "innerHTML", "<font color=red>Update done!</font>  <a href='list_schedule.php?loan_id=".$id."&applic_id=".$applic_id."&cust_name=".$cust_name."&cust_no=".$cust_no."&account_name=".$account_name."loan_officer=".$loan_officer."&from_year=".$from_year."&from_month=".$from_month."&from_mday=".$from_mday."&to_year=".$to_year."&to_month=".$to_month."&to_mday=".$to_mday."&status=".$status."&format=excel' target=blank()><b>Print Repayment Schedule</b></a>");
	}else
		$resp->alert("Loan not disbursed! The repayment schedule could not be printed!");
	return $resp;
}
...
function list_disbursed($cust_name, $cust_no, $account_name, $loan_officer, $from_date,$to_date, $cheque_no,$branch_id){
	...
	$officer_res = mysql_query("select * from officer order by first_name, last_name");
	...
	$sth = mysql_query("select d.id as id, d.automatic as automatic, applic.id as applic_id, m.first_name as first_name, m.last_name as last_name, m.mem_no as mem_no, o.first_name as of_first_name, o.last_name as of_last_name, a.account_no as account_no, a.name as account_name, d.id as id, d.amount as amount, d.date as date, d.balance as balance, d.period as loan_period, d.cheque_no as cheque_no, d.int_rate as int_rate, d.int_method as int_method, d.grace_period as grace_period, d.writeoff_period as writeoff_period, d.arrears_period as arrears_period, d.written_off as written_off, d.bank_account as bank_account from disbursed d join loan_applic applic on d.applic_id=applic.id join member m on applic.mem_id=m.id join loan_product p on applic.product_id=p.id join accounts a on p.account_id= a.id join officer o on applic.officer_id=o.id where (m.first_name like '%".$name."%' or m.last_name like '%".$name."%') and m.mem_no like '%".$mem_no."%' and d.cheque_no like '%".$cheque_no."%' and a.name like '%".$account_name."%' and ".$officer." and d.date >= '".$from_date."' and d.date <='".$to_date."' ".$branch." order by o.first_name, o.last_name, d.cheque_no, m.first_name, m.last_name");
	
	$sth2 = mysql_query("select d.id as id, d.automatic as automatic, applic.id as applic_id, m.first_name as first_name, m.last_name as last_name, m.mem_no as mem_no, o.first_name as of_first_name, o.last_name as of_last_name, a.account_no as account_no, a.name as account_name, d.id as id, d.amount as amount, d.date as date, d.penalty_rate as penalty_rate, d.balance as balance, d.period as loan_period, d.cheque_no as cheque_no, d.int_rate as int_rate, d.int_method as int_method, d.grace_period as grace_period, d.writeoff_period as writeoff_period, d.arrears_period as arrears_period, d.written_off as written_off, d.bank_account as bank_account from disbursed d join loan_applic applic on d.applic_id=applic.id join member m on applic.mem_id=m.id join loan_product p on applic.product_id=p.id join accounts a on p.account_id= a.id join officer o on applic.officer_id=o.id where (m.first_name like '%".$name."%' or m.last_name like '%".$name."%') and m.mem_no like '%".$mem_no."%' and d.cheque_no like '%".$cheque_no."%' and a.name like '%".$account_name."%' and ".$officer." and d.date >= '".$from_date."' and d.date <='".$to_date."' ".$branch." order by o.first_name, o.last_name, d.cheque_no, m.first_name, m.last_name");
...
function list_outstanding($cust_name, $cust_no, $account_name, $loan_officer,$from_date,$to_date, $cheque_no,$branch_id){
	...
	$content .= "<div class='panel-footer'>                 
                                ...
                                <button class='btn  btn-primary' onclick=\"xajax_list_outstanding(getElementById('cust_name').value, getElementById('cust_no').value, getElementById('account_name').value, getElementById('officer_id').value, getElementById('from_date').value, getElementById('to_date').value,getElementById('cheque_no').value,getElementById('branch_id').value);\">Search</button>
                            </div></div>
                        </form>
                        <!--/ Form default layout -->
                    </div></div>";
	...
	//CALCULATE DUE PRINCIPAL
		...
		$schedule = "<a href='list_schedule.php?loan_id=".$row['id']."&applic_id=".$row['applic_id']."&cust_name=".$cust_name."&cust_no=".$cust_no."&account_name=".$account_name."&loan_officer=".$loan_officer."&from_year=".$from_year."&from_month=".$from_month."&from_mday=".$from_mday."&to_year=".$to_year."&to_month=".$to_month."&to_mday=".$to_mday."&status=outstanding&format=excel' target=blank()>Schedule</a>";
...
function list_due($cust_name, $cust_no, $account_name,$loan_officer,$date,$cheque_no,$branch_id){
	...
	$sth = mysql_query("select d.id as id, applic.id as applic_id, m.first_name as first_name, m.last_name as last_name, m.mem_no as mem_no, o.first_name as of_first_name, o.last_name as of_last_name, a.account_no as account_no, a.name as account_name, d.id as id, d.cheque_no as cheque_no, d.amount as disbursed_amt, d.date as date, d.balance as balance, d.period as loan_period, d.arrears_period as arrears_period, d.int_rate as int_rate, d.int_method as int_method, d.next_princ_amt as next_princ_amt, d.next_int_amt as next_int_amt, d.next_pay_date as next_pay_date, date_format(d.last_pay_date, '%d') as last_mday, date_format(d.last_pay_date, '%m') as last_month, date_format(d.last_pay_date, '%Y') as last_year from disbursed d join loan_applic applic on d.applic_id=applic.id join member m on applic.mem_id=m.id join loan_product p on applic.product_id=p.id join accounts a on p.account_id= a.id join officer o on applic.officer_id=o.id where (m.first_name like '%".$name."%' or m.last_name like '%".$name."%') and m.mem_no like '%".$mem_no."%' and a.name like '%".$account_name."%' and ".$officer." and d.date <='".$date."' and d.balance >0 and d.cheque_no like '%".$cheque_no."%' and d.written_off=0 ".$branch." order by o.first_name, o.last_name, d.cheque_no, m.first_name, m.last_name");
	if(@ mysql_numrows($sth) == 0){
...
function add_recovered($written_id, $cust_name, $cust_no, $account_name, $loan_officer, $from_date, $to_date,$branch_id){
	$resp = new xajaxResponse();	
	$applic_res = mysql_query("select d.id as id, applic.id as applic_id, m.first_name as first_name, m.last_name as last_name, m.mem_no as mem_no, o.first_name as of_first_name, o.last_name as of_last_name, a.account_no as account_no, a.name as account_name, d.id as id, d.amount as disburse_amount, d.date as disburse_date, w.balance as write_balance,  d.bank_account as bank_account, w.amount as write_amount, w.date as write_date from disbursed d join loan_applic applic on d.applic_id=applic.id join member m on applic.mem_id=m.id join loan_product p on applic.product_id=p.id join accounts a on p.account_id= a.id join officer o on applic.officer_id=o.id join written_off w on w.loan_id=d.id where w.id='".$written_id."'");
...
function list_cleared($cust_name, $cust_no, $account_name, $loan_officer, $from_date,$to_date, $branch_id){ 
	...
	$sth = mysql_query("select d.id as id, applic.id as applic_id, m.first_name as first_name, m.last_name as last_name, m.mem_no as mem_no, o.first_name as of_first_name, o.last_name as of_last_name, a.account_no as account_no, a.name as account_name, d.id as id, d.amount as amount, d.date as date, d.balance as balance, (d.period/30) as loan_period,  DATEDIFF(d.last_pay_date, d.date) as actual_days from disbursed d join loan_applic applic on d.applic_id=applic.id join member m on applic.mem_id=m.id join loan_product p on applic.product_id=p.id join accounts a on p.account_id= a.id join officer o on applic.officer_id=o.id where (m.first_name like '%".$name."%' or m.last_name like '%".$name."%') and m.mem_no like '%".$mem_no."%' and a.name like '%".$account_name."%' and ".$officer." and d.date >= '".$from_date."' and d.date <='".$to_date."' and d.balance <=0 " .$branch." order by o.first_name, o.last_name");
...
$total_amount = mysql_fetch_assoc(mysql_query("select  sum(d.amount) as amount,sum(d.balance) as balance from disbursed d join loan_applic applic on d.applic_id=applic.id join member m on applic.mem_id=m.id join loan_product p on applic.product_id=p.id join accounts a on p.account_id= a.id join officer o on applic.officer_id=o.id where (m.first_name like '%".$name."%' or m.last_name like '%".$name."%') and m.mem_no like '%".$mem_no."%' and a.name like '%".$account_name."%' and ".$officer." and d.date >= '".$from_date."' and d.date <='".$to_date."' and d.balance =0 " .$branch." order by o.first_name, o.last_name"));
	while($row = mysql_fetch_array($sth2)){
		...
		$schedule = "<a href='list_schedule.php?loan_id=".$row['id']."&branch_id=".$branch_id."&applic_id=".$applic_id."&cust_name=".$cust_name."&cust_no=".$cust_no."&account_name=".$account_name."loan_officer=".$loan_officer."&from_year=".$from_year."&from_month=".$from_month."&from_mday=".$from_mday."&to_year=".$to_year."&to_month=".$to_month."&to_mday=".$to_mday."&status=".$status."&format=excel' target=blank()><b>Schedule</b></a>";
		//PENALTY
		$pen_res = mysql_query("select sum(amount) as amount from penalty where status='pending' and loan_id='".$row['id']."'");
		$pen = mysql_fetch_array($pen_res);
		$penalty = ($pen['amount'] == NULL) ? "--" : $pen['amount'];
...
//	$resp->assign("status", "innerHTML", "select p.id as id, p.begin_bal as begin_bal, p.loan_id as loan_id, p.date as date, p.princ_amt as princ_amt, p.int_amt as int_amt, (p.princ_amt + p.int_amt) as amount_paid, d.amount as disbursed_amt, p.bank_account as bank_account, b.min_balance as min_balance, b.account_balance as account_balance, date_format(d.date, '%Y') as disburse_year, date_format(d.date, '%m') as disburse_month, date_format(d.date, '%d') as disburse_mday  from payment p join disbursed d on p.loan_id=d.id join bank_account b on p.bank_account=b.id where p.id='".$pay_id."'");
	...
	$sth = mysql_query("select d.id as id, applic.id as applic_id, m.first_name as first_name, m.last_name as last_name, m.mem_no as mem_no, o.first_name as of_first_name, o.last_name as of_last_name, a.account_no as account_no, a.name as account_name, d.id as id, d.amount as disbursed_amt, d.date as date, d.balance as balance, d.period as loan_period, d.arrears_period as arrears_period, d.int_rate as int_rate, d.int_method as int_method, d.next_princ_amt as next_princ_amt, d.next_int_amt as next_int_amt, d.next_pay_date as next_pay_date, d.last_pay_date as last_pay_date, d.pay_freq as pay_freq, date_format(d.last_pay_date, '%d') as last_mday, date_format(d.last_pay_date, '%m') as last_month, date_format(d.last_pay_date, '%Y') as last_year from disbursed d join loan_applic applic on d.applic_id=applic.id join member m on applic.mem_id=m.id join loan_product p on applic.product_id=p.id join accounts a on p.account_id= a.id join officer o on applic.officer_id=o.id where d.id='".$loan_id."'");
	$row = mysql_fetch_array($sth);	
...
function insert_pay($loan_id, $disbursed_amt, $balance, $int_mode, $int_rate, $former_penalty, $penalty, $total_int,  $amount_paid, $mode, $bank_account_id, $receipt_no, $date, $princ_arrears, $int_arrears, $princ_due, $int_due,$cust_name,$cust_no, $status, $branch_id, $from_date,$to_date){
	...
	elseif($amount_paid <= 0)
		...
		if($calc->dateToDays($date) <= $calc->dateToDays($row['mday'], $row['month'], $row['year'])){
