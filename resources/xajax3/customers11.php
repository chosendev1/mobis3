...
function member_ledger($mem_id, $type, $from_date,$to_date){
	...
	$content .= "<div class='panel panel-default' id='demo'>
                            <div class='panel-heading'>
                        <center>
						<p><h3 class='semibold text-primary mt0 mb5'><B>".$branch['overall_name']."</B></h3></p>
						<p><h3 class='semibold text-primary mt0 mb5'><B>".$branch['branch_name']."</B></h3></p>
...
function insert_group($name, $group_no,$branch_id){
	$resp = new xajaxResponse();
	if($name!='' && $group_no!=''){
	$sth = mysql_query("select * from loan_group where name='".$name."' and branch_id='".$branch_id."'");
	$nosth = mysql_query("select * from loan_group where group_no='".$group_no."' and branch_id='".$branch_id."'");
	}
	if($branch_id==''){
...
$content .= '<div class="panel-body pt0 pb0">
	...
	$content .= '<div class="form-group">
                                           ...
                                           <input type="text" name="phone" id="phone"  maxlength=13 class="form-control" placeholder="e.g 25670199012">
											...
											$content .= '<div class="form-group">
                                            <label class="col-sm-3 control-label">NationalID No:</label>
                                            <div class="col-sm-6">
                                           <input type="text" name="natid" id="phone"  maxlength=13 class="form-control" placeholder="e.g 8959307234">
                                            </div></div>';
                                             $content .= '<div class="form-group">
                                            <label class="col-sm-3 control-label">Email:</label>
                                            <div class="col-sm-6">
                                           ...
                                           <input type="text" maxlength=13 name="kin_phone" id="kin_phone" placeholder="e.g 25670199012" class="form-control">
                                            ...
                                            <div class="col-sm-6">
                                           <select type=text name="sex" id="sex" class="form-control" required><option value=""><option value="M">M</option><option value="F">F</option></select>
                                            </div></div>';
                                             $content .= '<div class="form-group">
                                            <label class="col-sm-3 control-label">Date of Birth:</label>
                                            <div class="col-sm-6">
                                           <input type="text" name="dob" id="dob" class="form-control" placeholder="Select Date Of Birth">
                                            ...
                                            <div class="input-group">
                                                <span>
                                                    <div class="btn btn-primary">
                                                        <span class="icon iconmoon-file-3"></span><input type="file" name="photo" id="photo">
                                                    </div>
                                                </span>
                                            </div>
                                        </div></div>';                                            
                                            ...
                                            <div class="input-group">
                                               <span>
                                                    <div class="btn btn-primary">
                                                        <span class="icon iconmoon-file-3"></span><input type="file" name="sign" id="sign">
                                                    </div>
                                                </span>
                                            </div>
                                           ...
                                           <input type=hidden name="branch_no" id="branch_no" value='.$branch['branch_no'].'>
                                            ...
                                            $content .= '<div class="panel-footer"><button type="reset" class="btn btn-default" onclick=\"xajax_addCustForm(); return false;\">Reset</button>
                                            <input type="reset" value="Save" name="cust_enter" id="cust_enter" class="btn btn-primary">';
                                            $content .= '</div>';
...
function addCustomer($branch_no, $first_name, $last_name, $phone, $natid, $email, $paddress, $kin_name, $kin_phone, $dob_year, $dob_month, $dob_mday, $sex,$branch_id)
{
	$resp = new xajaxResponse();
	$calc = new Date_Calc();
	//$resp->alert("they called us");
	//return $resp;
	if($first_name == '' || $last_name=='' || $phone=='' || $natid=='' || $paddress=='' || $kin_name=='' || $kin_phone =='' || $sex=='')
	...
	elseif($calc->isFutureDate($dob_mday, $dob_month, $dob_year))
		...
		$new_cust = @mysql_query("INSERT INTO member (branch_no, first_name, last_name, telno, natid_no, email, address, kin, kin_telno, dob, sex,branch_id) VALUES ($branch_no, '".$first_name."', '".$last_name."', '".$phone."', '".$natid."', '".$email."', '".$paddress."', '".$kin_name."', '".$kin_phone."', '".$dob."', '".$sex."',".$branch_id.")");
		
		$action .= "INSERT INTO member (branch_no, first_name, last_name, telno, natid_no, email, address, kin, kin_telno, dob, sex) VALUES ($branch_no, '".$first_name."', '".$last_name."', '".$phone."', '".$natid."', '".$email."', '".$paddress."', '".$kin_name."', '".$kin_phone."', '".$dob."', '".$sex."')";
		...
		$content .= libinc::sendSMS($phone, "Welcome to ".$branch['branch_name']." Sacco, your member number is .".$mem_no);
	}	
	$resp->assign("status", "innerHTML", $content);
	return $resp;
}

function membersList($type, $mem_no, $mem_name,$phone, $natid,$start,$branch_id)
{
	$content ="";
	$resp = new xajaxResponse();
	
	
	$mem_no1 = ($mem_no == 'All') ? "" : "and mem_no like '%".$mem_no."%'";
	$tel_no1 = ($phone == 'All') ? "" : "and telno like '%".$phone."%'";
	$nat_id1 = ($natid == 'All') ? "" : "and natid_no like '%".$natid."%'";
	//$mem_name = ($mem_name == 'All') ? "" : "and mem_name like '%".$phone."%'";
	$branch = ($branch_id=='all') ? NULL : "and branch_id=".$branch_id;
	$resp->assign("status", "innerHTML", "");
	if (strtolower($type) == 'all'){
		$mem = @mysql_query("select id, mem_no, first_name, last_name, telno, natid_no, sex, sign_name, photo_name, (DATEDIFF(NOW(), dob)/365) as age, kin, kin_telno, address, status,  email, telno, natid_no from member where (first_name like '%".$mem_name."%' or last_name  like '%".$mem_name."%')  ".$nat_id1." ".$mem_no1." ".$tel_no1." ".$branch." order by first_name, last_name asc");
		$memb = @mysql_query("select id, mem_no, first_name, last_name, telno, natid_no, sex, sign_name, photo_name, (DATEDIFF(NOW(), dob)/365) as age, kin, kin_telno, address, status,  email, telno, natid_no from member where (first_name like '%".$mem_name."%' or last_name  like '%".$mem_name."%')  ".$nat_id1." ".$mem_no1." ".$tel_no1." ".$branch." order by first_name, last_name asc");
		
		$head = "CUSTOMERS";
	}elseif (strtolower($type) == 'members'){
		$mem = @mysql_query("select id, mem_no, first_name, last_name, telno, natid_no, sex, sign_name, photo_name, (DATEDIFF(NOW(), dob)/365) as age, kin, kin_telno, address, status,  email, telno, natid_no from member where id in (select mem_id from shares) and (first_name like '%".$mem_name1."%' or last_name  like '%".$mem_name1."%') and telno like '%".$tel_no1."%' and natid_no like '%".$nat_id."%' and mem_no like '%".$mem_no1."%' ".$branch." order by first_name, last_name asc");
		$memb = @mysql_query("select id, mem_no, first_name, last_name, telno, natid_no, sex, sign_name, photo_name, (DATEDIFF(NOW(), dob)/365) as age, kin, kin_telno, address, status,  email, telno, natid_no from member where id in (select mem_id from shares) and (first_name like '%".$mem_name1."%' or last_name  like '%".$mem_name1."%') and telno like '%".$tel_no1."%' and natid_no like '%".$nat_id."%' and mem_no like '%".$mem_no1."%' ".$branch." order by first_name, last_name asc ") or trigger_error(mysql_error());
		
		$head = "MEMBERS";
	}elseif (strtolower($type) == 'non_members'){
		$mem = @mysql_query("select id, mem_no, first_name, last_name, telno, natid_no, sex, sign_name, photo_name, (DATEDIFF(NOW(), dob)/365) as age, kin, kin_telno, address, status,  email, telno, natid_no from member where id NOT in (select mem_id from shares) and (first_name like '%".$mem_name1."%' or last_name  like '%".$mem_name1."%') and telno like '%".$tel_no1."%' and natid_no like '%".$nat_id."%' and mem_no like '%".$mem_no1."%' ".$branch." order by first_name, last_name asc") or trigger_error(mysql_error());
		$memb = @mysql_query("select id, mem_no, first_name, last_name, telno, natid_no, sex, sign_name, photo_name, (DATEDIFF(NOW(), dob)/365) as age, kin, kin_telno, address, status,  email, telno, natid_no from member where id NOT in (select mem_id from shares) and (first_name like '%".$mem_name1."%' or last_name  like '%".$mem_name1."%') and telno like '%".$tel_no1."%' and natid_no like '%".$nat_id."%' and mem_no like '%".$mem_no1."%' ".$branch." order by first_name, last_name asc ") or trigger_error(mysql_error());
		$head = "NON-MEMBERS";
	}
                      ...
                      <div class="form-group">
                                    <div class="row">
										...
										<div class="col-sm-6">
                                            ...
                                            <input type=text value="'.$phone.'"  id="telno"  class="form-control">
                                        </div>
										<div class="col-sm-6">
                                            <label class="control-label">NationalID No:</label>
                                            <input type=text value="'.$natid.'" id="natid_no" class="form-control">
                                        </div>
                                    </div>
                                </div>
...
$content .= '<div class="panel-body pt0 pb0">
  			  		<div class="form-group header bgcolor-default">
                                 		<div class="col-md-12">
                                                	<h4 class="semibold text-primary mt0 mb5">EDIT CUSTOMER DETAILS</h4>
                                               		 <p class="text-default nm"></p>
                                           	 </div>
                                        </div><div id="responsediv" class="alert alert-dismissable alert-danger"></div>';
                                            $content .= '<div class="form-group">
...
function mobileBankingSubscription(){
	...
	$content.= "<div class='panel-footer'>                              
                                ...
                                <input type='button' class='btn  btn-primary' value='Save'  onclick=\"xajax_subscription(getElementById('memid').value, getElementById('mem_no').value, getElementById('pin').value, getElementById('mm_no').value,return false;\">
                            </div>";
