...
function list_applics($cust_name, $cust_no, $account_name, $from_date,$to_date, $category,$branch_id,$search='',$group_id){
	...
	$content .='</select>                                           
		 ...
		 $content .= "<div class='panel-footer'>                              
                                ...
                                <input type='button' class='btn  btn-primary' id ='search' value='Search' onclick=\"xajax_list_applics(getElementById('cust_name').value, getElementById('cust_no').value, getElementById('account_name').value,getElementById('from_date').value, getElementById('to_date').value, '".$category."', getElementById('branch_id').value, getElementById('search').value,getElementById('group_id').value);\">
                            </div></div>
                        </form>
                        <!--/ Form default layout -->
                    </div></div>";
			...
			$content .= '<div class="panel panel-default" id="demo">
                            <div class="panel-heading">
                             <p><h3 class="semibold text-primary mt0 mb5">LIST OF '.$head.'</h3></p>';
                              ...
                              $content .=' <p><h5>LOAN OFFICER:&nbsp;<font color="#00BFFF">'.strtoupper($officer).'</font></h5></p>
...
list($year,$month,$mday) = explode('-', $date);
	...
	if($int_rate =='' || $int_method=='' || $grace_period=='' || $arrears_period=='' || $writeoff_period=='' || $loan_period=='' || $bank_account_id=='' || $penalty_rate=='')
		$resp->call("You may not leave any field blank");
	elseif(!$calc->isValidDate($mday, $month, $year))
		$resp->call("Disbursement rejected! Please enter valid date");
	elseif($calc->isFutureDate($mday, $month, $year))
		$resp->call("Disbursement rejected! You have entered a future date");
