...
function list_deposits($mem_no, $name, $product, $from_date,$to_date,$branch_id){
	...
	$sth2 = mysql_query("select d.receipt_no as receipt_no, d.cheque_no as cheque_no,d.status as status,d.transaction as transaction, d.bank_account as bank_account, d.id as id, d.percent_value as percent_value, d.flat_value as flat_value, m.mem_no as mem_no, m.first_name as first_name, m.last_name as last_name, d.amount as amount, d.date as date, a.account_no as account_no, a.name as name from deposit d join mem_accounts mem on d.memaccount_id=mem.id join  member m on mem.mem_id=m.id join savings_product s on mem.saveproduct_id=s.id join accounts a on s.account_id=a.id where mem.saveproduct_id like '%".$product."%' and (m.mem_no like '%".$dmem_no."%' and (m.first_name like '%".$dname."%' or m.last_name like '%".$dname."%')) and d.date >= '".$from_date."' and d.date <= '".$to_date."' and s.type='free' ".$branch." order by m.first_name, m.last_name, d.trans_date");
		if(@mysql_numrows($sth)==0){
			...
			$content .= '<div class="panel panel-default" id="demo">
                            <div class="panel-heading">
                                <p><h5 class="semibold text-primary mt0 mb5">LIST OF DEPOSITS1</h5></p>
                                
                               </div>';
 		$content .= '<table class="table table-striped table-bordered" id="table-tools">';
 		
			$content .= "<thead><th>#</th><th><b>Member Name</b></th><th><b>MemberNo</b></th><th><b>Credit</b></th><th><b>ReceiptNo</b></th><th><b>Cheque No</b></th><th><b>Percent Charge</b></th><th><b>Flat Charge</b></th><th><b>Date</b></th><th><b>Product</b></th><th><b>Bank Account</b></th>
			<th><b>Transaction</b></th>
			<th><b>Status</b></th></thead><tbody>";
			$i=$stat+1;
			while($row = @mysql_fetch_array($sth2)){
				...
				$content .= "<tr><td>".$i."</td><td>".$row['first_name']. " ".$row['last_name']."</td><td>".$row['mem_no']."</td><td>".$edit."</td><td>".$row['receipt_no']."</td><td>".$cheque_no."</td><td>".number_format($row['percent_value'], 2)."</td><td>".number_format($row['flat_value'], 2)."</td>
				
				<td>".$row['date']."</td><td>".$row['account_no']." - ".$row['name']."</td><td>".$bank['account_no']." - ".$bank['name']."</td>
