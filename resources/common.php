...
{
	$user_id = CAP_Session::get("userId");
	if ($user_id == '')
		return ;
	else
	{
		$res = mysql_query("INSERT into logs set user_id='".$user_id."', time=NOW(),  action='". mysql_escape_string($action)."', msg = '".mysql_escape_string($msg)."'");
	}
}
...
//COMMON SP FUNCTIONS 
function get_savings_bal($mode, $year, $month, $mday)
{
	/*
	*AFFECTED TABLES: shares, payment, other_income, deposit, withdrawal, 
	*
	*/
	//GET MIN BALANCE
	$date = sprintf("%d-%02d-%02d 23:59:59", $year, $month, $mday);
			...
			//echo("select min_bal from savings_product where id = (select saveproduct_id from mem_accounts where id = ".$mode.")");
			$min_bal = $min_row['min_bal'];
			//ACCOUNT BALANCE
				$dep_res = mysql_query("select sum(amount - flat_value - percent_value) as amount from deposit where memaccount_id='".$mode."' and date <= '".$date."'");
				$dep = mysql_fetch_array($dep_res);
				$dep_amt = ($dep['amount'] != NULL) ? $dep['amount'] : 0;
				//WITHDRAWALS
				$with_res = mysql_query("select sum(amount + flat_value + percent_value) as amount from withdrawal where memaccount_id='".$mode."' and date <= '".$date."'");
				$with = mysql_fetch_array($with_res);
				$with_amt = ($with['amount'] != NULL) ? $with['amount'] : 0;
				//MONTHLY CHARGES 
				$charge_res = mysql_query("select sum(amount) as amount from monthly_charge where memaccount_id='".$mode."' and date <= '".$date."'");
				$charge = mysql_fetch_array($charge_res);
				$charge_amt = ($charge['amount'] != NULL) ? $charge['amount'] : 0;
				//INTEREST AWARDED
				$int = mysql_fetch_array(mysql_query("select sum(amount) as amount from save_interest where memaccount_id='".$mode."' and date <= '".$date."'"));
				$int_amt = ($int['amount'] != NULL) ? $int['amount'] : 0;
				//LOAN REPAYMENTS
				$pay_res = mysql_query("select sum(princ_amt + int_amt) as amount from payment where mode='".$mode."' and date <= '".$date."'");
				$pay = mysql_fetch_array($pay_res);
				$pay_amt = ($pay['amount'] != NULL) ? $pay['amount'] : 0;
				//INCOME DEDUCTIONS
				$inc_res = mysql_query("select sum(amount) as amount from other_income where mode='".$mode."' and date <= '".$date."'");
				$inc = mysql_fetch_array($inc_res);
				$inc_amount = ($inc['amount'] != NULL) ? $inc['amount'] : 0;
				//SHARES DEDUCTIONS
				$sha_res = mysql_query("select sum(value) as amount from shares where mode='".$mode."' and date <= '".$date."'");
...
function month_array($str, $year, $month){
	$content = "<select class='form-control' name='".$str."year' id='".$str."year'>";
		if($year<>"")
			$content .= "<option value='".$year."'>" . $year;
		else
			$content .= "<option value='".date('Y')."'>" . date("Y");
		for($i=1900; $i <= date("Y")+10; $i++)
			$content .= "<option value='".$i."'>" . $i;
	$content .= "</select>";
	$content .= "<select class='form-control' name='".$str."month' id='".$str."month'>";
	if($month<>"")
		$content .= "<option value='".$month."'>" . substr(digit_month($month), 0, 1). strtolower(substr(digit_month($month), 1, 2));
	else
		$content .= "<option value='".date("m")."'>" . date("M");
...
function mday($str, $mday){
	...
	$content = "<select class='form-control' name='".$str."mday' id='".$str."mday'><option value='".$mday."'>".$mday;
    for($i=1; $i<32; $i++){
         $x=sprintf("%02d", $i);
...
function branch(){
	...
	if(libinc::isCompanyUser())
		$branch = mysql_query("select branch_no,branch_name from branch where companyId='".libinc::getCompanyId()."'");
	$content = "<select name='branch_id' id='branch_id' class='form-control'>";
...
function branch_rep($branch_id){
	...
	$content = "<select name='branch_id' id='branch_id' class='form-control'><option value='all' label=' --All Branches-- '> --All Branches-- ";
	while($row = mysql_fetch_assoc($branch)){
		$content .="<option value='".$row['branch_no']."' label='".$row['branch_name']."'>".$row['branch_name'];
	}
	$content .="</select>";
	...
	$content = "<select name='branch_id' id='branch_id' class='form-control'>";
	while($row = mysql_fetch_assoc($branch)){
		$selected = $branch_id == $row['branch_no'] ? "SELECTED" : NULL;
		$content .="<option value='".$row['branch_no']."' label='".$row['branch_name']."' ".$selected.">".$row['branch_name'];
	}
	$content .="</select>";*/
...
$content = "Records Per Page:<select name='num' id='num' class='form-control'><option value='".$num_rows."' >".$num_rows."</option>";
$num = 10;
while($num<=400){
...
$content = "<div align=center>";
	...
	function createMemberAccount($mem_no){
		...
		$password = hash('SHA512',$row['mem_no']);
