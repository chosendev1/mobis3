	...
	public function withdraw($memaccount, $amount, $number){
		...
		elseif($date < $row['open_date'])  //No withdrawal before the account was opened
			...
			elseif($free_amt < $charge['min_bal'])
				 ...
				 */
				require_once("MM.php");
				//$memaccount, $amount, $number
				$saccoId = getItemById("branch",$branch_id,"branch_no","companyId");
				$mmval = MM::makeWithdraw($number,$amount,$saccoId,$memaccount);
				if($mmval){
					...
					//////////////////
					commit();
					$resp = "Withdrawal of ".$amount." successfull! transaction code is:".$receipt_no." Byonic Code:".$mmval;
				}
				else{
					$resp = "Withdraw of amount ".$amount." failed!\n MM Error is:".$mmval;
					rollback();
				}
			}
		}
