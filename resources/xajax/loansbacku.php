...
function other_applic($stuff, $loan_type,$entry_mode){
	...
	$content .= '<div class="form-group panel bgcolor-default">
                ...
                $content .= '<div class="form-group"><div class="col-sm-6">
                                          ...
                                          $content .= "<input type=hidden id='group_id' value='".$group_id."'><select id='mem_id' class='form-control' onchange=\"xajax_guarantor('".$group_id."', getElementById(\'mem_id\').value);\"><option value=''>Choose</option>";
      while($row = @mysql_fetch_array($sth)){
...
list($year,$month,$mday) = explode('-', $date);
	...
	//	$based_on = $prod['based_on'];
		...
		*/
		...
		if(@ mysql_numrows($sth) > 0){
			$resp->assign("display_div", "innerHTML", "<font color=green>Disbursement done!</font><a href='export/schedule?loan_id=".$loan['last_id']."&applic_id=".$applic_id."&format=excel'><b>Print Repayment Schedule</b></a>");
		}else
...
$from_date = sprintf("%04d-%02d-%02d 00:00:00", $from_year, $from_month, $from_mday);
	...
	//CALCULATE DUE PRINCIPAL
		...
		$schedule = "<a href='export/schedule/".$row['id']."/".$row['applic_id']."/excel'>View</a>";
