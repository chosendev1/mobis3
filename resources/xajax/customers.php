...
$xajax->registerFunction("getRank");
...
function uploadCustomersForm(){
	...
	$content .= '
         ...
         $content .='<div class="col-sm-6"><div class="form-group">
                    ...
                    <span><select class="form-control" name="company" id="company" onchange="xajax_loadBranches(this.value);" >
...
function loadGroups($branchId,$customerType){
	...
	$content .= '<div class="span3">
		...
		<span><select class="form-control" name="group_id" id="group_id" required>
...
function memsavings_ledger($mem_no, $mem_id, $save_acct, $from_date, $to_date)
{
	...
	$content .= '<div class="panel panel-default" id="demo">
                            <div class="panel-heading">
                        <p><h5 class="semibold text-primary mt0 mb5">INTERIM SAVINGSm STATEMENT</p>                          
                            </div>';
	...
	if((strtolower($acc_row['transaction']) == 'deposit') || (strtolower($acc_row['transaction']) == 'loan disbursed')){
		...
		if(strtolower($acc_row['transaction']) == 'withdrawal'){
			$charge = @mysql_fetch_array(@mysql_query("select voucher_no, cheque_no, (flat_value + percent_value) as amounted from withdrawal where id='".$acc_row['id']."'"));
			//$charge_amt = ($charge['amount'] != NULL) ? $charge['amount'] : 0;
				$charge_amt = $charge['amounted'];
...
function member_ledger($mem_id, $type, $from_date,$to_date,$group_id){
  ...
  $to_date = $today;
                                    ...
                                    <div class="row">
                                     <div class="col-sm-3">
                                            <label class="control-label">Date Range:</label>
                                             <div class="col-xs-6"><input type="text" class="form-control" id="from_date" name="from_date" placeholder="From" /></div>
                                              <div class="col-sm-6"><input type="text" class="form-control" id="to_date" name="to_date" placeholder="to" /></div>
                                        </div>
                                        <div class="span3">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <span class="input-group-btn">
                                                <button class="btn btn-info" type="button" onclick=\'xajax_member_ledger(document.getElementById("memid").value, "mem_id", document.getElementById("from_date").value,document.getElementById("to_date").value); return false;\'>Show</button>
                                            </span>
                                        </div>
                                    </div>
                                        ...
                                        <div class="input-group">
                                            ...
                                            <span class="input-group-btn">
                                                <button class="btn btn-info" type="button" onclick=\'xajax_member_ledger(document.getElementById("mem_no").value, "mem_no", document.getElementById("from_date").value,document.getElementById("to_date").value); return false;\'>Show</button>
                                            </span>
                                        </div>
                                    </div>                
                                    ';
                                     ...
                                     <div class="span3">
                                           ...
                                           <select name="groupId" id="groupId" class="form-control">'.$groups.'</select>
                                            <span class="input-group-btn">
                                                 ...
                                                 <button class="btn btn-info" type="button" onclick=\'xajax_member_ledger("", "", document.getElementById("from_date").value,document.getElementById("to_date").value,document.getElementById("groupId").value); return false;\'>Show</button>
                                            </span>
                                        </div> </div>                                     
	...
	$content .= "<div class='panel panel-default' id='demo'>
                            <div class='panel-heading'>
                        <center>
		  <h2 class='semibold text-primary mt0 mb5'>MEMBER STATEMENT</h2></center>
...
<h3 class='semibold'>Branch : ".$branch['branch_name']."</h3>
		   <h5 class='semibold'><B>Name:</B><b>".strtoupper($mem_row['first_name'])." ". strtoupper($mem_row['last_name'])."</b></h5></p><p><h5 class='semibold'><B>Member Number:</B><b>".$mem_row['mem_no']."</b></h5></p><p><h5 class='semibold'><B>Period:</B><b>".$from_date  ."  -  ". $to_date."</b></h5></p></td>

		   <td align=right><img src='signs/".$mem_row['sign_name']."?dummy=".time()."' width=90 height=90></td></tr></table><br>
			</div>";
 ...
 <div class="col-sm-12">
        ...
        <div class="panel-body"> 
                            ...
                            <li class="active"><a data-toggle="tab" href="#menu1">General Transctions</a></li>
  <li><a data-toggle="tab" href="#home">Shares</a></li>
  ...
  <div class="panel panel-default" id="demo">
 		...
 		$content .= '<table class="table table-striped" id="table-tools">';
  ...
  <div class="panel panel-default" id="demo">
                            <div class="panel-heading">
                        <p><h5 class="semibold text-primary mt0 mb5">MEMBER STATEMENT</p>                          
                            </div>';
        $content .= "<table class='table table-striped' id='table-tools'>";                    
                            
 		$content .= "<thead>
			<th>Date</th><th>Description</th><th>Debit</th><th>Credit</th><th>Balance</th></thead><tbody>";
		   $content .= "<tr>		    
			<td>Before".$start_date."</td><td align=center>B/F</td><td align=center>--</td><td align=center>--</td><td align=center>".number_format($cumm_save, 2)."</td>
		    </tr>
		    ";
	$acc_res = @mysql_query("select id, date, amount, transaction from deposit where memaccount_id = '".$save_acct."' and date > '".$start_date."' and date <='".$end_date."' UNION select id, date, amount, transaction from withdrawal where memaccount_id = $save_acct and date > '".$start_date."' and date <= '".$end_date."' UNION select id, date, amount, 'Loan Disbursement' as transaction from disbursed where mode = $save_acct and date >='".$start_date."' and date <='".$end_date."' UNION select id, date, amount, transaction from monthly_charge where memaccount_id = $save_acct and date >'".$start_date."' and date <='".$end_date."' UNION select id, date, amount, transaction from save_interest where memaccount_id = $save_acct and date > '".$start_date."' and date <='".$end_date."' UNION select id, date, princ_amt + int_amt + penalty + other_charges as amount, transaction from payment where mode= '".$save_acct."' and date > '".$start_date."' and date <='".$end_date."' UNION select id,date,value as amount, transaction from shares where mode = '".$save_acct."' and date > '".$start_date."' and date <= '".$end_date."' UNION select id, date, amount, transaction from other_income where mode = '".$save_acct."' and date > '".$start_date."' and date <= '".$end_date."' UNION select id,date,amount, transaction from payable where mode= '".$save_acct."' and date >= '".$start_date."' and date <= '".$end_date."' order by date asc");
	...
	while ($acc_row = @mysql_fetch_array($acc_res))
	{
		...
		if(strtolower($acc_row['transaction']) == 'shares'){
			...
			//$charge_amt = ($charge['amount'] != NULL) ? $charge['amount'] : 0;
			$descr="Share Purchase - SHR:".$charge['receipt_no'];
	...
	if(strtolower($acc_row['transaction']) == 'loan fees payable'){
		...
		if($tot_disbursement != 0){
			...
			$content .= "
		    <tr>
		    <td>".$acc_row['date']."</td><td>".$descr."</td><td>--</td><td>".number_format($tot_disbursement, 2)."</td><td>".number_format($cumm_save, 2)."</td>
		    </tr>
		    ";
		}
		...
		if($tot_processing_inc > 0){
			...
			$content .= "
		    <tr>
			<td>".$acc_row['date']."</td><td>".$descr."</td><td>".number_format($tot_processing_inc, 2)."</td><td>--</td><td>".number_format($cumm_save, 2)."</td>
		    </tr>
		    ";
		}
		...
		if($tot_charge_payable!=0){
			...
			$content .= "
		    <tr>
			<td>".$acc_row['date']."</td><td>".$descr."</td><td>".$tot_charge_payable."</td><td>--</td><td>".$cumm_save."</td>
		    </tr>
		    ";
		}
		...
		if($charge_amt !=0){
			...
			$content .= "
		    <tr>
			<td>".$acc_row['date']."</td><td align=center>Transactional Charge</td><td align=center>".number_format($charge_amt, 2)."</td><td align=center>--</td><td align=center>".number_format($cumm_save,2)."</td>
		    </tr>
		    ";
		}
		if($tot_inc >0){
		 ...
		 if($tot_inc2 !=0){
			...
			$content .= "
		    <tr>
			<td>".$acc_row['date']."</td><td>".$descr."</td><td>".number_format($tot_inc2, 2)."</td><td>".number_format($cumm_save, 2)."</td>
		    </tr>
		    ";
		}
		 ...
		 if($tot_payable!=0){
			...
			$content.= "
		    <tr>
			<td>".$acc_row['date']."</td><td>Health Insurance</td><td>".number_format($tot_payable,2)."</td><td>--</td><td>".number_format($cumm_save,2)."</td>
		    </tr>
		    ";
		}
		...
		if($tot_fees >0){
			...
			$content .= "
		    <tr>
			<td>".$acc_row['date']."</td><td align=center>Monthly Charge</td><td align=center>".number_format($tot_fees,2)."</td><td align=center>--</td><td align=center>".$cumm_save."</td>
		    </tr>
		    ";
		}
	...
	$content .= "</tbody></table></div></div>";
		      ...
		      $content .='<div id="menu1" class="tab-pane fade in active"><font color="red">This Member Has No Savings Account!</font><br></div>';
  ...
  <div class="panel panel-default" id="demo">
	...
	$content .='<div class="col-sm-12">
        ...
        <div class="panel-body"> 
                            <ul class="nav nav-tabs">
  <li class="active"><a data-toggle="tab" href="#home">Shares</a></li>
  <li><a data-toggle="tab" href="#menu1">Savings</a></li>
  <li><a data-toggle="tab" href="#menu3">Loans</a></li>
</ul>
	<div class="tab-content">
...
';
  ...
  <div class="panel panel-default" id="demo">
	...
	$content .= '<div id="menu1" class="tab-pane fade">
	<div class="panel panel-default" id="demo">
                            <div class="panel-heading">
	...
	/*$over_res = @mysql_query("select * from member where mem_no='".$mem_no."'");
		...
		*/
		...
		$content .= ' <div id="menu3" class="tab-pane fade"><div class="panel panel-default" id="demo">
                            <div class="panel-heading">
	...
	$content .='<div id="menu3" class="tab-pane fade"><font color="red">The group Has No Members!</font><br></div>';
...
function add_group(){
	...
	$content = '
		...
		<input type=button class='btn btn-primary' name='submit' id='submit' value='Save' onclick=\"xajax_insert_group(getElementById('group_name').value, getElementById('group_no').value, getElementById('branch_id').value);\"><div id='status'></div></div>";                             
...
function insert_group($name, $group_no,$branch_id){
	...
	if($branch_id==''){
		...
		$resp->assign("status", "innerHTML", "<font class='alert alert-danger'>Select a branch</font>");
		return $resp;
	}elseif($name==''){
		//$resp->alert("You may not leave group name blank");
		$resp->assign("status", "innerHTML", "<font class='alert alert-danger'>You may not leave group name blank</font>");
		return $resp;
	}elseif($group_no==''){
		//$resp->alert("You may not leave group number blank");
		$resp->assign("status", "innerHTML", "<font class='alert alert-danger'>You may not leave group number blank</font>");
		return $resp;		
	}elseif(mysql_num_rows($sth) > 0){
		//$resp->alert("");
		$resp->assign("status", "innerHTML", "<font class='alert alert-danger'>Group Name Exists</font>");
		return $resp;
	}elseif(mysql_num_rows($nosth) > 0){
		//$resp->alert("");
		$resp->assign("status", "innerHTML", "<font class='alert alert-danger'>Group Number Exists</font>");
		return $resp;	
	}else{
		//$resp->alert($_SESSION['members']);
		//return $resp;
		...
		commit();
		$resp->assign("status", "innerHTML", "<font class='alert alert-success'>Group Created!</font>");
	}
	return $resp;
}
...
$content = '
	...
	if($stuff ==''){
		...
		if(@ mysql_numrows($sth) == 0)
			...
			$content = '<div class="col-md-12"><div class="panel panel-default"><div class="panel-heading">
                                <h3 class="panel-title">List Of Groups Found</h3>
                            </div>
                            <table class="table table-hover" id="row-detail">';
 		$content .= '<th>#</th><th>GROUP NAME</th><th>MEMBERS</th></tbody>';
	...
	if($stuff ==''){
		...
		if(@ mysql_numrows($sth) == 0){
			...
			$resp->assign("status", "innerHTML", $cont);
 			...
 			$content .= '<div class="panel panel-default" id="demo">
                            <div class="panel-heading">
							   <h4 class="panel-title">LIST OF GROUPS</h4>
                             </div><div class="panel-body">   
                            ';
 		$content .= '<table class="table table-hover" id="table-tool1s">';
 		$content .= '<th>#</th><th>GROUP NAME</th><th>GROUP NUMBER</th><th>BRANCH</th><tbody>';	
			$x=1;
			while($row = @mysql_fetch_array($sth)){
				//$color = ($x%2 == 0) ? "white" : "lightgrey";
...
function edit_photos($mem_no){
	...
	$content .="<form enctype='multipart/form-data' action='customers/editPhoto' method=post enctype='multipart/form-data' class='panel panel-default'>";
		
           	$content .='<div class="panel-heading">
                              <h3 class="panel-title">  
 <h4 class="semibold text-primary">EDIT PHOTO AND SIGNATURE OF MEMBER : '.strtoupper($row['first_name']).' '.strtoupper($row['last_name']). ' - '.$mem_no. '</h4>                                           	 
	...
	<div class="span6">
                           <label class="control-label">Photo:'.$mem_no.'</label>
...
$content .= '<div class="panel-body pt0 pb0">
  			  		<div class="form-group header bgcolor-default">
                                 		<div class="col-md-12">
                                                	<h4 class="semibold text-primary mt0 mb5">REGISTER CUSTOMER11</h4>
                                               		 <p class="text-default nm"></p>
                                           	 </div>
...
function addCustFormvvv()
{
   ...
   $content ="";
	$content .="<form method='post' action='customers/regCustomer' enctype='multipart/form-data' class='panel panel-default'>";
$content .= '
  			  		<div class="panel-heading">
                              <h3 class="panel-title">   
                                                	<h4 class="semibold text-primary">REGISTER CUSTOMER</h4>
		  ...
		  <div class="row-form">
							    <div class="span5">
                           <label class="control-label ">Branch:</label>
                 <span><select class="form-control" name="branch_id" id="branch_id" required>
                    	<option value="">--Select Branch--</option>
                    	'.libinc::getItem("branch","branch_no","branch_name","").'
                    </select>
                   </div>'; 
         ...
         $content .='  <div class="span5">
                     ...
                     <span><select class="form-control" name="customer_type" id="customer_type" onchange=\'xajax_loadGroups(getElementById("branch_id").value,getElementById("customer_type").value);\' required>
					...
					$content .='  <div class="span5">
                               <label class="control-label">Title/Rank:</label>
                     <select class="form-control" name="rank" id="rank" required>
                    	<option value="">--Select Rank--</option>
                    	'.libinc::getItem("ranks","position","rank","").'
                    </select>
                    </div></div>';/*later added title/rank drop down field to this form*/
	...
	<div class="row-form">
                           ...
                           <label class="control-label">First Name:</label>
                                           ...
                                           <input type="int" name="first_name" id="first_name" class="form-control" required>
                                            </div>';                                            
                                             $content .= ' <div class="span5">
                           <label class="control-label">Last Name:</label>
                                           
                                           <input type="int" name="last_name" id="last_name" class="form-control" required>
                                            </div></div>';
                                             $content .= ' <div class="row-form">
							    <div class="span5">
                           <label class="control-label">Phone No:</label>
                                         
                                           <input type="int" name="phone" id="phone"  maxlength=13 class="form-control" placeholder="e.g 25670199012">
                                            </div>';
                                             $content .= ' 
							    <div class="span5">
                           <label class="control-label">Email:</label>
                                         
                                           <input type="int" id="email" name="email" class="form-control" placeholder="e.g example@domain.com">
                                            </div></div>';
                                             $content .= ' <div class="row-form">
							    <div class="span5">
                           <label class="control-label">Physical Address:</label>
                                         
                                           <input type="int" name="paddress" id="paddress" class="form-control">
                                            </div>';
                                             $content .= ' 
							    <div class="span5">
                           <label class="control-label">Next of Kin Name:</label>
                                         
                                           <input type="int" name="kin_name" id="kin_name" class="form-control" >
                                            </div></div>';
                                             $content .= ' <div class="row-form">
							    <div class="span5">
                           <label class="control-label">Next of Kin Phone No:</label>
                                        
                                           <input type="int" maxlength=13 name="kin_phone" id="kin_phone" placeholder="e.g 25670199012" class="form-control">
                                            </div>';
                                             $content .= ' 
							    <div class="span5">
                           <label class="control-label">Gender:</label>
                                          
                                           <select type=text name="sex" id="sex" class="form-control" required><option value="">--Select--<option value="M">Male<option value="F">Female<option value="I">Institution
                                           </select>
                                            </div></div>';
                                             $content .= ' <div class="row-form">
							    <div class="span5">
                           <label class="control-label">Date of Birth:</label>
                                           ...
                                           <input type="int" name="dob" id="dob" class="form-control" placeholder="Select Date Of Birth">
                                            </div>';
                                            $resp->call("createDOB",'dob');
                                            // $resp->call("createDate","date");
                                            $content .= ' 
							    <div class="span5">
                           <label class="control-label">Photo File:</label>
                           ...
                           <label class="control-label">Signature File:</label>
                                                        ...
                                                        <span class="icon iconmoon-file-3"></span> Browse <input type="file" name="sign" id="photo"> 
...
$content .= '
  			  		<div class="panel-heading">
                              <h3 class="panel-title">   
                                <h4 class="semibold text-primary">REGISTER CUSTOMER</h4>
                                        ...
                                        </div></div><div class="block-fluid"><div class="panel-body">';
     	  $content .='
		  ...
		  <div class="row-form ">
							    <div class="span3">
                           <label class="control-label required">Branch:<font color="red" size="2px">*</font></label>
                 <span><select class="form-control" name="branch_id" id="branch_id" required>
                    	<option value="">Select Branch</option>
                    	'.libinc::getItem("branch","branch_no","branch_name","").'
                    </select>
         ...
         $content .='  <div class="span3">
                               <label class="control-label">Customer Type:<font color="red" size="2px">*</font></label>
                     <span><select class="form-control" name="customer_type" id="customer_type" onchange=\'xajax_loadGroups(getElementById("branch_id").value,getElementById("customer_type").value);\' required>
	...
	<div class="row-form ">
							    <div class="span3">
                           <label class="control-label">First Name:<font color="red" size="2px">*</font></label>
                                           
                                           <input type="int" name="first_name" id="first_name" class="form-control" required>
                                            </div>';                                            
                                             $content .= ' <div class="span3">
                           <label class="control-label">Last Name:<font color="red" size="2px">*</font></label>
                                           
                                           <input type="int" name="last_name" id="last_name" class="form-control" required>
                                            </div>
                                            <div class="span3">
                           <label class="control-label">Rank/Title:<font color="red" size="2px">*</font></label>
                                          <select class="form-control" name="rank" id="rank" required>
                    	<option value="">Choose Rank/Title</option>
                    	'.libinc::getItem("ranks","position","rank","").'
                    </select>
                                            </div></div>';
        $content .= '<div class="row-form ">
				 <div class="span3">
                           <label class="control-label">IPPS/Ref No:</label>
                                           
                                           <input type="int" name="ippsNo" id="ippsNo" class="form-control" placeholder="Optional">
                                            </div>
                                  <div class="span3">
                           <label class="control-label required">Phone No:<font color="red" size="2px">*</font></label>
                                         
                                           <input type="int" name="phone" id="phone"  maxlength=13 class="form-control" placeholder="e.g 25670199012" required>
                                            </div>
                                             <div class="span3">
                           <label class="control-label">Email:</label>
                            ...
                            $content .= ' <div class="row-form ">
							    <div class="span3">
                           <label class="control-label">Physical Address:<font color="red" size="2px">*</font></label>
                                         
                                           <input type="int" name="paddress" id="paddress" class="form-control" required>
                                            </div>
                                            <div class="span3">
                           <label class="control-label">Next of Kin Name:<font color="red" size="2px">*</font></label>
                                         
                                           <input type="int" name="kin_name" id="kin_name" class="form-control" required>
                                            </div>
                                   
				<div class="span3">
                           <label class="control-label">Next of Kin Phone No:<font color="red" size="2px">*</font></label>
                                        
                                           <input type="int" maxlength=13 name="kin_phone" id="kin_phone" placeholder="e.g 25670199012" class="form-control" required>
                                            </div></div>';
                           $content .= ' <div class="row-form "> 
							    <div class="span3">
                           <label class="control-label">Gender:<font color="red" size="2px">*</font></label>
                                          
                                           <select type=text name="sex" id="sex" class="form-control" required><option value="">--Select--<option value="M">Male<option value="F">Female<option value="I">Institution
                                           </select>
                                            </div>
				 <div class="span3">
                           <label class="control-label">Date of Birth:<font color="red" size="2px">*</font></label>
                                            
                                           <input type="int" name="dob" id="dob" class="form-control" placeholder="Select Date Of Birth" required>
                                            </div>
						...
						$content .= '<div class="span2">
                           <label class="control-label">Signature File:</label>                                    
                                                 ...
                                                 <input type="file" name="sign" id="photo"> 																						 
                                                    </div>
                                            </div>
...
function membersList($type, $mem_no, $ipps_no='All', $mem_name='All', $start,$branch_id)
{
	$content ="";
	$resp = new xajaxResponse();
	$mem_no1 = ($mem_no == 'All'|| $mem_no=='') ? "" : $mem_no;
	$ipps = ($ipps_no=='All' || $ipps_no=='') ? NULL : "and ipps=".$ipps_no;
	$mem_name1 = ($mem_name == 'All' || $mem_name=='') ? "" : $mem_name;
	$branch = ($branch_id=='all' || $branch_id=='') ? NULL : "and branch_id=".$branch_id;
	$resp->assign("status", "innerHTML", "");
	if (strtolower($type) == 'all'){
		$mem = @mysql_query("select id, r_id,mm_no, mem_no, first_name, last_name, sex, sign_name, photo_name, dob, (DATEDIFF(NOW(), dob)/365) as age, kin, kin_telno, address, status,  email, telno,ipps,computerNo from member where (first_name like '%".$mem_name1."%' or last_name  like '%".$mem_name1."%') and mem_no like '%".$mem_no1."%' ".$ipps." ".$branch." order by r_id, first_name, last_name asc");
		$memb = @mysql_query("select id, r_id,mm_no,mem_no, first_name, last_name, sex, sign_name, photo_name, dob, (DATEDIFF(NOW(), dob)/365) as age, kin, kin_telno, address, status,  email, telno,ipps,computerNo from member where (first_name like '%".$mem_name1."%' or last_name  like '%".$mem_name1."%') and mem_no like '%".$mem_no1."%' ".$ipps." ".$branch." order by r_id, first_name, last_name asc");
		$head = "CUSTOMERS";
	}elseif (strtolower($type) == 'members'){
		//$mem = @mysql_query("select id, r_id,mm_no, mem_no, first_name, last_name, sex, sign_name, photo_name, (DATEDIFF(NOW(), dob)/365) as age, kin, kin_telno, address, status,  email, telno,ipps,computerNo  from member where id in (select mem_id from shares) and (first_name like '%".$mem_name1."%' or last_name  like '%".$mem_name1."%') and mem_no like '%".$mem_no1."%' ".$branch." order by r_d, first_name, last_name asc");
		$mem = @mysql_query("select id, r_id,mm_no, mem_no, first_name, last_name, sex, sign_name, photo_name, dob, (DATEDIFF(NOW(), dob)/365) as age, kin, kin_telno, address, status,  email, telno,ipps,computerNo from member where (first_name like '%".$mem_name1."%' or last_name  like '%".$mem_name1."%') and mem_no like '%".$mem_no1."%' ".$ipps." ".$branch." order by r_d, first_name, last_name asc");
		//$memb = @mysql_query("select id, r_id,mm_no, mem_no, first_name, last_name, sex, sign_name, photo_name, (DATEDIFF(NOW(), dob)/365) as age, kin, kin_telno, address, status,  email, telno,ipps,computerNo  from member where id in (select mem_id from shares) and (first_name like '%".$mem_name1."%' or last_name  like '%".$mem_name1."%') and mem_no like '%".$mem_no1."%' ".$branch." order by r_id, first_name, last_name asc") or trigger_error(mysql_error());
		$memb = @mysql_query("select id, r_id,mm_no, mem_no, first_name, last_name, sex, sign_name, photo_name, dob, (DATEDIFF(NOW(), dob)/365) as age, kin, kin_telno, address, status,  email, telno,ipps,computerNo from member where (first_name like '%".$mem_name1."%' or last_name  like '%".$mem_name1."%') and mem_no like '%".$mem_no1."%' ".$ipps." ".$branch." order by r_id, first_name, last_name asc") or trigger_error(mysql_error());
		
		$head = "MEMBERS";
	}elseif (strtolower($type) == 'non_members'){
		$mem = @mysql_query("select id, r_id,mm_no,mem_no, first_name, last_name, sex, sign_name, photo_name, dob, (DATEDIFF(NOW(), dob)/365) as age, kin, kin_telno, address, status,  email, telno,ipps,computerNo from member where id NOT in (select mem_id from shares) and (first_name like '%".$mem_name1."%' or last_name  like '%".$mem_name1."%') and mem_no like '%".$mem_no1."%' ".$branch." order by r_id, first_name, last_name asc") or trigger_error(mysql_error());
		$memb = @mysql_query("select id, r_id, mm_no,mem_no, first_name, last_name, sex, sign_name, photo_name, dob, (DATEDIFF(NOW(), dob)/365) as age, kin, kin_telno, address, status,  email, telno,ipps,computerNo from member where id NOT in (select mem_id from shares) and (first_name like '%".$mem_name1."%' or last_name  like '%".$mem_name1."%') and mem_no like '%".$mem_no1."%' ".$branch." order by r_id, first_name, last_name asc") or trigger_error(mysql_error());
		$head = "NON-MEMBERS";
	}
            ...
            <div class="col-md-12">                
                ...
                <div class="widget">
                    <div class="head">
                        ...
                        <h2>SEARCH FOR&nbsp;'.$head.'</h2>
                          
                               
                                          
                            <div class="panel-body">
                            
							 <div class="row-foraam">
							    <div class="span4">
                           <label class="control-label">Customer Name:</label>
                         
                                <input type="text" value="'.$mem_name.'" id="mem_name" class="validate[required,creditCard]"/>
						  ...
						  <div class="span2">
                               <label class="control-label">Member No:</label>
                                ...
                                <input type="text" value="'.$mem_no.'" id="mem_no" class="validate[required,creditCard]"/>
                               
                            </div>
                            		  <div class="span2">
                              <label class="control-label">Ipps/Ref No:</label>
                          
                                <input type="text" value="'.$ipps_no.'" id="ipps" class="validate[required,creditCard]"/>
                               
                            </div>
                                <div class="span3">
                  
                             <label class="validate[required,creditCard]">Branch:</label>                                        
                                        '.branch_rep($branch_id).'
                                        </div>
                        </div> 
						';                          
                                                      	
	$content .= '<div class="span4"><div class="">                              
                                
                                <input type="button" class="btn  btn-primary" value="Show" id="submit"  onclick=\'xajax_membersList("'.$type.'", getElementById("mem_no").value,getElementById("ipps").value,getElementById("mem_name").value, "1",getElementById("branch_id").value)\'>
                            </div></div>
                        </form>
                        <!--/ Form default layout -->
	...
	{				    	    
		...
		$ippsNo= libinc::getItemById('company',CAP_session::get('companyId'),'companyId','cno');	 
		$content.= '<table class="borderless table-hover" id="table-tools" width="100%">';
 		$content .= '<thead>';
 		
 		$content .= ' <th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th></thead><tbody>';
...
if(trim($members['photo_name']) == ''){
$pho ="<td><img class='img-circle' src='".base_url()."resources/themes/mobis/img/user.png' width=50 height=15> </td>";
}else{
$pho ="<td><img class='img-circle' src='photos/".$members['photo_name']."' width=40 height=15></td>";
}
			//end photo test
			...
			$content.= "
				    <tr>
				     ";
				      ...
				      $content.= "
				       <td><b>Name:&nbsp</b>";
				       if(!empty(libinc::getItemById('ranks',$members[r_id],'position','rank')))
				       				       
				       $content.=libinc::getItemById('ranks',$members[r_id],'position','rank')."&nbsp"; 
				       
				       $content.="$members[first_name] $members[last_name] 
				       <br><b>MemberNo:</b> $members[mem_no]
				       <br><b>Sex:</b>  $members[sex]
				       <br><b>Date of Birth:</b>  $members[dob]
				       <br><b>PhoneNo:</b>  $members[telno]
				       <br>";
				       if(!empty($ippsNo)){
				       $content.= "<b>IppsNo:</b> $members[ipps]<br>";
				       $content.= "<b>ComputerNo:</b> $members[computerNo]";
				       }
				       $content.= "</td>
				       <td><b>Address:</b> $members[address]
				       <br><b>Next of Kin:</b> $members[kin] 
				       <br><b>Next of kin PhoneNo:</b> $members[kin_telno]
				       </td>
				      				      
				       <td><b>Shares: ".number_format(libinc::numShares($members['id']),1)."<br>";
				       if(empty(libinc::memaccountId($members['id'])))
				        $content.= "<b>Savings Balance:</b> 0";
				       else
				        $content.= "<b>Savings Balance:</b> ".number_format(libinc::get_savings_bal(libinc::memaccountId($members['id'])))."</td>";
				       
					/*$content.="
				        <td><a href='javascript:;' onclick=\"xajax_insertDeposit('".$members['mem_no']."'); return false;\">Deposit</a><br>*/
				    $content.="
				        <td><a href='javascript:;' onclick=\"xajax_add_withdrawal('".$members['mem_no']."'); return false;\">Withdrawal</a><br>                           
					<a href='javascript:;' onclick=\"xajax_editCustForm('".$members['id']."', '".$type."', '".$mem_no."', '".$mem_name."', '".$start."','".$branch_id."'); return false;\">Edit Data</a><br>
					<a href='javascript:;' onclick=\"xajax_edit_photos('".$members['mem_no']."'); return false;\">Edit Photo</a></td>";
					$content.= $pho."
					 <td><img  src='signs/".$members['sign_name']."' width=40 height=15></td>
				      </tr>  ";
				      
                                   
					$i++;/*we pick some fields and list all members data*/
		}
	...
	//return $resp;
		...
		$cont = "<font color=red>No Results Found.</font>";
		$resp->assign("status", "innerHTML", $cont);
	//return $resp;
	}elseif(strtolower($type) == 'non_members'){
...
function editCustForm($id, $type, $mem_no, $mem_name, $start,$branch_id)
{
	$resp = new xajaxResponse();
	if(CAP_Session::get('edit')<>1){
	$resp->alert('You Dont Have Permissions to Edit');
	...
	$mem = @mysql_fetch_array(@mysql_query("select * from member where id='".$id."'"));
	$resp->assign("status", "innerHTML", "");
	
	
if(trim($mem['photo_name']) == ''){
	  $img = "<img class='img-circle' src='".base_url()."resources/themes/mobis/img/user.png' width=80 height=60>";

}else{
 	  $img = '<img class="img-circle" src="photos/'.$mem['photo_name'].'" width=80 height=60>';
}
	
	$app = get_instance();
	$mem_n = $app->uri->segment(3);
	if($mem_n)
	{
	$content.="<form method='post' action='customers/editCustomer' class='panel panel-default form-bordered'>";
                                                                                     	
		$content .= '
  			  		<div class="panel-heading">
                              <h3 class="panel-title">   
                                <h4 class="semibold text-primary">EDIT CUSTOMER</h4>
	...
	<div class="row-form ">
                           ...
                           <label class="control-label">First Name:</label>
                                           ...
                                           <input type="int" name="first_name" id="first_name" class="form-control" value="'.$mem['first_name'].'"  >
                                            </div>';                                            
                                             $content .= ' <div class="span3">
                           <label class="control-label">Last Name:</label>
                                           
                                           <input type="int" name="last_name" id="last_name" class="form-control" value="'.$mem['last_name'].'" >
                                            </div>
                                            <div class="span3">
                           <label class="control-label">Rank/Title:</label>
                                          <select class="form-control" name="rank" id="rank" >
                    	<option value="'.$mem['r_id'].'">'.libinc::getItemById('ranks',$mem['r_id'],'position','rank').'</option>
                    	'.libinc::getItem("ranks","position","rank","").'
                    </select>
                                            </div></div>';
        $content .= '<div class="row-form ">
				 <div class="span3">
                           <label class="control-label">IPPS No:</label>
                                           
                                           <input type="int" name="ippsNo" id="ippsNo" class="form-control" value="'.$mem['ipps'].'">
                                            </div>
                                  <div class="span3">
                           <label class="control-label ">Phone No</label>
                                         
                                           <input type="int" name="phone" id="phone"  maxlength=13 class="form-control" value="'.$mem['telno'].'" >
                                            </div>
                                             <div class="span3">
                           <label class="control-label">Email:</label>
                                         
                                           <input type="int" id="email" name="email" class="form-control" value="'.$mem['email'].'">
                                            </div></div>';
                           ...
                           <label class="control-label">Physical Address:</label>
                                           ...
                                           <input type="int" name="paddress" id="paddress" class="form-control" value="'.$mem['address'].'" >
                                            </div>
                                            <div class="span3">
                           <label class="control-label">Next of Kin Name:</label>
                                         
                                           <input type="int" name="kin_name" id="kin_name" class="form-control" value="'.$mem['kin'].'" >
                                            </div>
                                   
				<div class="span3">
                           <label class="control-label">Next of Kin Phone No:</label>
                                        
                                           <input type="int" maxlength=13 name="kin_phone" id="kin_phone" value="'.$mem['kin_telno'].'" class="form-control" >
                                            </div></div>';
                           $content .= ' <div class="row-form "> 
							    <div class="span3">
                           <label class="control-label">Gender:</label>
                                          
                                           <select type=text name="sex" id="sex" class="form-control" ><option value="'.$mem['sex'].'">'.$mem['sex'].'<option value="M">Male<option value="F">Female<option value="I">Institution
                                           </select>
                                            </div>
				 <div class="span3">
                           <label class="control-label">Date of Birth:</label>
                                            
                                           <input type="int" name="dob" id="dob" class="form-control" value="'.$mem['dob'].'" >
                                            </div></div>';
                                             $content .= '
                                            <input type=hidden name="branch_no" id="branch_no" value='.$branch['branch_no'].'>';
                                            $resp->call("createDOB",'dob');
                                                                                        
                                            $content .="<div class='panel-footer'><button type='reset' class='btn btn-default' onclick=\"xajax_membersList('".$type."', '".$mem_no."', '".$mem_name."', '".$start."','".$branch_id."')\">Back</button>&nbsp;<button type='button' class='btn btn-default' onclick=\"xajax_deleteCustomer('".$id."');\">Delete</button>&nbsp;<button type='button' class='btn btn-primary' onclick=\"xajax_updateCustomer('".$id."',document.getElementById('first_name').value, document.getElementById('last_name').value, document.getElementById('phone').value, document.getElementById('email').value, document.getElementById('paddress').value, document.getElementById('kin_name').value, document.getElementById('kin_phone').value, getElementById('dob').value,getElementById('sex').value,getElementById('rank').value,getElementById('ippsNo','".$mem_no."').value); return false;\">Update</button></div>";
...
function updateCustomer($id,$first_name, $last_name, $phone, $email, $paddress, $kin_name, $kin_phone, $dob, $sex, $rank, $ipps,$mem_no)
{
	...
	$calc = new Date_Calc();
	// if($first_name == '' || $last_name=='' || $phone=='' || $paddress=='' || $kin_name=='' || $kin_phone =='' || $sex==''|| $email=='' || $rank=='')

	if($first_name == '' || $last_name=='' || $sex=='')

	$resp->alert("You may not leave Names and Sex blank");
	elseif(!$calc->isValidDate($dob_mday, $dob_month, $dob_year))
		$resp->alert("Customer rejected! Please enter valid date $dob");
	elseif($calc->isFutureDate($dob_mday, $dob_month, $dob_year))
		$resp->alert("Customer rejected! You have entered a future date");
	else{
			...
			$dob = sprintf("%d-%02d-%02d", $dob_year, $dob_month, $dob_mday);
			$resp->confirmCommands(1, "Do you really want to update?");
			$resp->call("xajax_update2Customer", $id,$first_name, $last_name, $phone, $email, $paddress, $kin_name, $kin_phone, $dob, $sex,$rank,$ipps,$mem_no);
...
function update2Customer($id,$first_name, $last_name, $phone, $email, $paddress, $kin_name, $kin_phone, $dob, $sex,$rank, $ipps,$mem_no){
	$resp = new xajaxResponse();
	if(CAP_Session::get('edit')<>1){
	$resp->alert('You Dont Have Permissions to Edit');
	return $resp;
	}
	if(@mysql_query("update member set first_name='".$first_name."', last_name='".$last_name."', telno='".$phone."', email='".$email."', kin='".$kin_name."', kin_telno='".$kin_phone."', dob='".$dob."', address='".$paddress."', sex='".$sex."',r_id='".$rank."', mm_no='".$ipps."' where id='".$id."'")){
		
		$action = "update member set first_name='".$first_name."', last_name='".$last_name."', telno='".$phone."', email='".$email."', kin='".$kin_name."', kin_telno='".$kin_phone."', dob='".$dob."', address='".$paddress."', sex='".$sex."',r_id='".$rank."', mm_no='".$ipps."' where id='".$id."'";
		$msg = "Update member details: set Mem_no: ".$mem_no.", First Name: ".$first_name.", Last Name: ".$last_name."";
		log_action($_SESSION['user_id'], $action, $msg);
		$resp->call('xajax_membersList','all',$mem_no,$ipps,'',1);
		$resp->alert("Customer Details Updated Successfully!");
	}else
		...
		$content = '
                    <div class="col-md-12">
                      ...
                      <div class="form-group">
                                      ...
                                      <label class="control-label"><br>OR Enter Member No:</label>
