...
function manage_chart($type){
	...
	$content = '<div class="panel panel-default" id="demo">
                            <div class="panel-heading">
                                <h3 class="panel-title">MANAGE CHART OF ACCOUNTS</h3>
			...
			while($level3 = @ mysql_fetch_array($level3_res)){
				...
				while($level4 = @ mysql_fetch_array($level4_res)){
					...
					if($level4['name']=='Compulsory Savings'){
						...
						$content .= "<li><a href='#' onclick=\"xajax_edit_account('".$level4['id']."', '".$level4['account_no']."', '".$level4['name']."', '".$type."');\">".$level4['account_no']." - ".$level4['name']."</a></li>";
					...
					while($level5 = @ mysql_fetch_array($level5_res)){
						...
						if($level5['account_no'] =='311102')
							...
							$content .= "<li><a href='#' onclick=\"xajax_edit_account('".$level5['id']."', '".$level5['account_no']."', '".$level5['name']."', '".$type."');\">".$level5['account_no']." - ".$level5['name']."</a></li>";
						...
						while($level6 = @ mysql_fetch_array($level6_res)){
							$content .= "<li><a href='#' onclick=\"xajax_edit_account('".$level6['id']."', '".$level6['account_no']."', '".$level6['name']."', '".$type."');\">".$level6['account_no']." - ".$level6['name']."</a></li>";
...
function add_account($last_no, $category, $cat_name){
	...
	$min = ($last_no >= 111101) ? ($category * 100) + 1 : ($category * 10) + 1;
	$now = $min;
	...
	if((($min +9) == ($last_no + 1) && $last_no < 111101) || (($min +99) == ($last_no + 1) && $last_no >= 111101)){
		$next_no = '';
		for($i=$min; $i <= $last_no; $i++){
...
$content .= '<div class="panel-body pt0 pb0">
  			  		<div class="form-group header bgcolor-default">
                                 		<div class="col-md-12">
                                                	<h4 class="semibold text-primary mt0 mb5">CREATE NEW ACCOUNT UNDER '.$category.' - '.$cat_name.'</h4>
        ...
        $content .= "<div class='panel-footer'><button type='reset' class='btn btn-default' onclick=\"xajax_manage_chart('".substr($category, 0, 1)."')\">Reset</button>
...
$content .= '<div class="panel-body pt0 pb0">
  			  		<div class="form-group header bgcolor-default">
                                 		<div class="col-md-12">
                                                	<h4 class="semibold text-primary mt0 mb5">EDIT ACCOUNT '.$account_no.'</h4>
                                               		 <p class="text-default nm"></p>
                                           	 </div>
                                        </div>';
        ...
        $content .= '<div class="form-group">
                                            <label class="col-sm-3 control-label">Account Name:</label>
                                            <div class="col-sm-6"><input type=hidden id="account_id"  name="account_id" value="'.$account_id.'"/><input type="hidden" id="account_no" name="account_no" value="'.$account_no.'"/><input type="text" id="account_name" name="account_name" value="'.$account_name.'" class="form-control"><input type=hidden id="next_no" name="next_no" value="'.$next_no.'" /></div>                                            
                                            </div>'; 
                                            
        $content .= "<div class='panel-footer'><button type='button' class='btn btn-default' onclick=\"xajax_manage_chart('".$type."')\">Back</button>
