...
{       $fixed_acc='';
	...
	$level1 = @mysql_query("select id, account_no, name from accounts where account_no like '".$prefix."%' 
	     					and account_no >= ".$prefix."1 and account_no <=".$prefix."9 and
						    name not like '%Bad Debt Expense on Loans%' and name not like '%Dividends Paid on Member Shares%'");
	while ($level1row = @mysql_fetch_array($level1))
	{
		$level2 = @mysql_query("select id, account_no, name from accounts where account_no like '".$level1row['account_no']."%' 
								and account_no >= ".$level1row['account_no']."01 and account_no <= ".$level1row['account_no']."99 
								and name not like 'Bad Debt Expense on Loans' and name not like '%Dividends Paid on Member Shares%'");		
		
		if (@mysql_num_rows($level2) > 0) // there's a level2 account
		{
			while ($level2row = @mysql_fetch_array($level2))
			{
				$level3 = @mysql_query("select id, account_no, name from accounts where account_no like '".$level2row['account_no']."%'
				 and account_no >= ".$level2row['account_no']."01 and account_no <= ".$level2row['account_no']."99
				  and id NOT in (select account_id from fixed_asset) and name not like 'Bad Debt Expense on Loans' 
				  and name not like '%Dividends Paid on Member Shares%'");	
	...
	//if (strtolower($_SESSION['position']) == 'manager')
		$acc = @mysql_query("select ac.name, ac.account_no as account_no, ba.id as bank_acct_id, ba.bank as bank_name from accounts ac 
								join bank_account ba on ac.id = ba.account_id ");	
	/*else
...
{             
	...
	$res = @mysql_query("select  r.bank_account, ac.name, r.id, r.amount, r.account_id, r.voucher_no, date_format(date, '%Y') as year, date_format(date, '%m') as month, date_format(date, '%d') as mday,r.contact as contact,r.details as details from  expense r join accounts ac on ac.id = r.account_id where r.id = $pid");
	if (@mysql_num_rows($res) > 0)
	{
...
$content .= '<div class="panel-body pt0 pb0">
  			  		<div class="form-group header bgcolor-default">
                                 		<div class="col-md-12">
                                                	<h4 class="semibold text-primary mt0 mb5">EDIT EXPENSES</h4>
                                               		 <p class="text-default nm"></p>
                                           	 </div>
                                        </div>';
                                         ...
                                         $content .= '<div class="form-group">
                                           ...
                                           <span>'.$row['name'].'</span>
                                            ...
                                            <label class="col-sm-3 control-label">Amount Paid:</label>
                                            <div class="col-sm-6">
                                          <input type="numeric" id="amount" name="amount" value="'.$row['amount'].'" class="form-control">
                                            </div></div>';
...
function updateExpense2($pid, $pname, $prev_bank_acct_id, $prev_amount, $amount, $voucher_no, $bank_acct_id, $date, $type, $contact, $details,$branch_id)
{       
         list($year,$month,$mday) = explode('-', $date);
       // list($to_year,$to_month,$to_mday) = explode('-', $to_date);
	$resp = new xajaxResponse();
	...
	else
	{
		...
		$resp->call('xajax_updateExpense', $pid, $pname, $prev_bank_acct_id, $prev_amount, $amount, $voucher_no, $bank_acct_id, $date, $type,$contact, $details, $branch_id);
	}
	return $resp;
}

function updateExpense($pid, $pname, $prev_bank_acct_id, $prev_amount, $amount, $voucher_no, $bank_acct_id, $date, $type,$contact, $details, $branch_id)
{	
	list($year,$month,$mday) = explode('-', $date);
	$resp = new xajaxResponse();
	$date = sprintf("%d-%02d-%02d", $year, $month, $mday);
	...
	{    //SAME BANK ACCOUNT AND AMOUNT
		$res = @mysql_query("update expense set date='".$date."', amount=$amount, voucher_no='".$voucher_no."', contact='".$contact."', details='".$details."' where id=$pid");
