		...
		<div class="panel-body">
 		<div class="form-group">
                                        ...
                                        <div class="col-sm-3">
                                            ...
                                            <select id="loan_type" class="form-control" onchange=\'xajax_loan_type(this.value);\'>
											
	                                         
									'.$option.'		
							<option value="">select<option value="Individual">Individual<option value="Group">Group</select>          
                                        </div>                                        
...
function loan_type($loan_type){
	...
	if($loan_type=='Individual'){
		...
		$content = "
                                            <label class='control-label'>OR Select Member No:</label><div class='input-group'><select id='mem_id' class='form-control'><option value=''>Select Member</option>";
		...
		$content .= "</select> <span class='input-group-btn'>
                                                    <div class='btn btn-primary btn-file'>
                                                        <span class='icon iconmoon-file-3' onclick=\"xajax_other_applic(getElementById('mem_id').value, '".$loan_type."', 'select');\"> Next</span>
                                                    </div>
                                                </span>
                                         </div></div></div>";
...
function other_applic($stuff, $loan_type,$entry_mode){
	...
	$content .= '<div class="form-group panel bgcolor-default">
                                       <div class="col-md-12"><hr>
                                       <div class="alert alert-info"><span class="bold">Group Name:&nbsp;&nbsp;</span><span class="text-primary">'.$group_name.'</span>
		</div>
                                            ';
                ...
                $content .= '<div class="form-group"><div class="col-sm-6">
                                          ...
                                          $content .= "<input type=hidden id='group_id' value='".$group_id."'><select id='grp_id' class='form-control' onchange=\"xajax_guarantor('".$group_id."', getElementById(\'mem_id\').value);\"><option value=''>Choose</option>";
      while($row = @mysql_fetch_array($sth)){
		...
		$content .= '
                                       <div class="col-md-12">
                                       <hr>
                                        <div class="alert alert-info"><span class="bold">Member Name:&nbsp;&nbsp;</span><span class="text-primary">'.ucwords($mem_name).'</span>
		</div>
	             ...
	             $content .= '<div class="col-sm-6"><div class="form-group">
	                                   ...
	                                   <br>
                                           ...
                                           <select id="marital_status" class="form-control"><option value="">Select<option value="Single">Single<option value="Married">Married</select>
                                            </div>
...
<br>
	...
	if($loan_type=='Individual'){
		...
		$content .= '<div class="form-group">
	                                   <div class="col-sm-12">
	                                   <br>
                                            <label class="control-label">Guarantor Type:</label>
		        ...
		        $content .= "<option value=''>Choose";
			$content .= "<option value='mems'>Members";
			$content .= "<option value='nonmems'>Non Members";
	...
	$content .= '</select></div>                                   
	                                   ...
	                                   <br>
                                           ...
                                           <input type="text" class="form-control" id="date" name="date" value="'.date('Y-m-d').'" /><br><br>
                                            </div></div>';
	$resp->call("createDate","date");
...
function insert_applic($group_id, $mem_id, $marital_status, $residence, $telno, $address, $income1, $income2, $amount, $based_on, $guarantor1, $guarantor2, $reason, $product_id, $officer_id, $date,$branch_id,$non_member_guarantor1,$guarantor1_phone,$non_member_guarantor2,$guarantor2_phone){
	$resp = new xajaxResponse();
	...
	if($mem_id=='' || $marital_status=='' || $residence=='' || $telno=='' || $address=='' || $income1=='' || $income2=='' || $amount=='' || $based_on == '' || $reason=='')
		...
		if($based_on == 'savings'){
			...
			if(!empty($guarantor1) && !empty($guarantor2)){
		...
		if(!empty($non_member_guarantor1) && !empty($guarantor1_phone) && !empty($non_member_guarantor2) && !empty($guarantor2_phone)){
		mysql_query("insert into non_member_guarantors set guarantor1name='".$non_member_guarantor1."', guarantor1_phone_no='".$guarantor1_phone."',guarantor2name='".$non_member_guarantor2."', guarantor2_phone_no='".$guarantor2_phone."',loan_id=$insertId");
		}
		//////////////////////
		...
		log_action($_SESSION['user_id'],$action,$msg);
	///////////////////////
	        $resp->alert("Application registered!");
...
function list_applics($cust_name, $cust_no, $account_name, $from_date,$to_date, $category,$branch_id,$search='',$group_id){
...
//start time	
	...
	while($row = mysql_fetch_array($sth2)){
		...
		if(strcmp($former_officer, $officer) != 0){
 		...
 		$content .= '<table class="borderless table-striped" id="table-tools">';
 		
 		$content .= '<thead><th>#</th><th>NAME</th><th>MEMBER No.</th><th>AMOUNT</th><th>REASON</th><th>COLLATERAL</th><th>GUARANTORS</th><th>DATE</th><th>LOAN BALANCE</th><th>EDIT COLLATERAL</th><th>APPROVE</th><th>DISBURSE</th></thead><tbody>';
		...
		if($row['disbursed_id'] != NULL){
			...
			$approving = "<a href='javascript:;' onclick=\"xajax_confirm_approve_applic('Approve','".$row['applic_id']."', '".$row['approved']."',    '".$cust_name."', '".$cust_no."', '".$account_name."', '".$from_date."','".$to_date."', '".$category."','".$branch_id."','','".$group_id."');\">Approve</a>";
		else
		...
		if($row['disbursed_id'] == NULL && $row['approved'] ==1)
			$disburse="<a href='javascript:;' onclick=\"xajax_confirm_disbursement('".$row['applic_id']."', '".$row['amount']."', '".$possible_amt."', '".$row['based_on']."','".$branch_id."');return false;\">Disburse</a>";
		else
			$disburse = "--";
		//$color = ($i%2 == 0) ? "lightgrey" : "white";
		
		$content .="<tr><td>".$i."</td><td>".$row['first_name']." ".$row['last_name']."</td><td>".$row['mem_no']."</td><td>".number_format($row['amount'], 2)."</td><td>".$row['reason']."</td><td>".$sub."</td><td>".$guar."</td><td>".$row['date']."</td><td>".number_format($loan_balance, 2)."</td><td>".$edit."</td><td>".$approving."</td><td>".$disburse."</td></tr>";
		$i++;
		$amt_sub_total += $row['amount'];
	}
	
	$content .= "<tr><td><b>TOTAL</b></td><td></td><td></td><td><b>".number_format($amt_sub_total, 2)."</b></td><td></td><td></td><td></td><td></td></td><td></td><td></td><td></td><td></tr>";
	$content .= "</tbody></table></div>";
...
function confirm_disbursement($applic_id, $amount, $possible_amt, $based_on, $branch_id){
	$resp = new xajaxResponse();
	$resp->confirmCommands(1, "Do you REALLY want to Disburse");
	$resp->call('xajax_add_disbursed',$applic_id, $amount, $possible_amt, $based_on, $branch_id);
	return $resp;
}
//DISBURSE LOAN
...
list($year,$month,$mday) = explode('-', $date);
	...
	//	$resp->call("Disbursement rejected!\n Repayment frequency must be divisible by the loan period");
		...
		$sth = mysql_query("select * from disbursed where applic_id='".$applic_id."'");
		if(@ mysql_numrows($sth) > 0){
			$resp->call("This loan has already been disbursed");
			return $resp;
		}
...
function list_due($cust_name, $cust_no, $account_name,$loan_officer,$date,$cheque_no,$branch_id,$group_id){
  ...
  $to_date = $today;
	...
	$amt_sub_total = 0; $bal_sub_total = 0; $princ_arrears_sub_total = 0; $int_arrears_sub_total = 0; $tot_arrears_sub_total; $penalty_sub_total = 0; $princ_due_sub_total = 0; $int_due_sub_total = 0; $tot_amt_sub_total = 0;
 		...
 		$content .= '<table class="borderless table-hover" id="table-tools">';
 		$content .= '<thead><th>#</th><th>NAME</th><th>MEMBER N0.</th><th>DATE</th><th>CHEQUE NO.</th><th>AMOUNT</th><th>LOAN BALANCE</th><th>PRINC ARREARS</th><th>INT ARREARS</th><th>TOTAL ARREARS</th><th>PENALTY</th><th>PRINC DUE</th><th>INT DUE</th><th>TOTAL AMOUNT DUE</th><th>SCHEDULE</th><th>PAY</th></thead><tbody>';
 		while($row = mysql_fetch_array($sth2)){
		$officer = $row['of_firstName']." ".$row['of_lastName'];
		//if(strcmp($former_officer, $officer) != 0){
			$former_officer = $officer;
	...
	//CALCULATE DUE PRINCIPAL
		...
		//$color = ($i%2 == 0) ? "lightgrey" : "white";
		$content .= "<tr><td>".$i."</td><td>".$row['first_name']." ".$row['last_name']."</td><td>".$row['mem_no']."</td><td>".$row['date']."</td><td>".$row['cheque_no']."</td><td>".number_format($row['disbursed_amt'], 2)."</td><td>".number_format($row['balance'], 2)."</td><td>".number_format($princ_arrears, 2)."</td><td>".number_format($int_arrears, 2)."</td><td>".number_format($total_arrears, 2)."</td><td>".number_format($penalty, 2)."</td><td>".number_format($due_princ, 2)."</td><td>".number_format($due_int, 2)."</td><td>".number_format($total_due,2)."</td><td>".$schedule."</td><td>".$pay."</td></tr>";
		$i++;
	...
	// PRINT SUB TOTALS
	$content .= "<tr><td><b>TOTAL</b></td><td></td><td></td><td></td><td></td><td><b>".number_format($amt_sub_total, 2)."</b></td><td><b>".number_format($bal_sub_total, 2)."</b></td><td><b>".number_format($princ_arrears_sub_total, 2)."</b></td><td><b>".number_format($int_arrears_sub_total, 2)."</b></td><td><b>".number_format($tot_arrears_sub_total, 2)."</b></td><td><b>".number_format($penalty_sub_total, 2)."</b></td><td><b>".number_format($princ_due_sub_total, 2)."</b></td><td><b>".number_format($int_due_sub_total, 2)."</b></td><td><b>".number_format($tot_amt_sub_total, 2)."</b></td><td></td><td></td></tr></tbody></table></div>";
	//}
	$resp->call("createTableJs");
	$resp->assign("display_div", "innerHTML", $content);
	return $resp;
...
$from_date = sprintf("%04d-%02d-%02d 00:00:00", $from_year, $from_month, $from_mday);
	...
	$total_amount = mysql_fetch_assoc(mysql_query("select sum(d.amount) as amount,  sum(d.balance) as balance from disbursed d join loan_applic applic on d.applic_id=applic.id join member m on applic.mem_id=m.id join loan_product p on applic.product_id=p.id join accounts a on p.account_id= a.id join employees o on applic.officer_id=o.employeeId where (m.first_name like '%".$name."%' or m.last_name like '%".$name."%') and m.mem_no like '%".$mem_no."%' ".$branch." and a.name like '%".$account_name."%' and ".$officer." ".$group." and d.date >= '".$from_date."' and d.date <='".$to_date."' and d.balance > 0 and d.written_off='0'".$state." ".$branch." order by o.firstName, o.lastName, m.first_name, m.last_name"));
		...
		//if(strcmp($former_officer, $officer) != 0){
 		...
 		$content .= '<thead><th>#</th><th>NAME</th><th>MEMBER NO.</th><th>DATE</th><th>AMOUNT</th><th>AMOUNT PAID</th><th>AMOUNT PREPAID</th><th>PRINC ARREAS</th><th>INT ARREARS</th><th>PENALTY</th><th>PRINC DUE</th><th>INT DUE</th><th>OUTSTANDING BALANCE</th><th>ACTION</th></thead><tbody>';
		while($row = mysql_fetch_array($sth2)){
		$officer = $row['of_firstName']." ".$row['of_lastName'];
			$former_officer = $officer;						
		//}
...
$content .= '<div class="panel-heading">
                                        ...
                                        </div><div class="panel-body"><div class="form-group"><div class="col-sm-6">';
...
$content .= '<div class="form-group">
	                                   <div class="col-sm-6">
                                           ...
                                           <select id="guarantor1" class="form-control"><option value="">';
                                 if($entry_mode == 'select'){
			$sth1 = mysql_query("select * from member where id<>'".$group_id."' order by first_name, last_name, mem_no");
			$sth2 = mysql_query("select * from member where id<>'".$group_id."' order by first_name, last_name, mem_no");
		}elseif($entry_mode =='enter'){
		  ...
		  $content .='<input type="hidden" id="guarantor1Name" value="">
               ...
               $content .= '<div class="form-group">
	                                   ...
	                                   <div class="col-sm-6">
                                            ...
                                            <label class="control-label">Guarantor1 Name:</label>
                                           <input type="text" id="guarantor1Name" class="form-control">
                                            </div>
                                            
                                           <div class="col-sm-6"> 
                                            <label class="control-label">Phone Number:</label>
                                           <input type="text" id="guarantor1Phone" class="form-control">
               ...
               $content .= '<div class="form-group">
	                                   ...
	                                   <div class="col-sm-6">
                                            ...
                                            <label class="control-label">Guarantor2 Name:</label>
                                           <input type="text" id="guarantor2Name" class="form-control">
                                            </div>
                                            
                                           <div class="col-sm-6"> 
                                            <label class="control-label">Phone Number:</label>
                                           <input type="text" id="guarantor2Phone" class="form-control">
					    <input type="hidden" id="guarantor1" value="">
					    <input type="hidden" id="guarantor2" value="">
                                           </div></div>';
