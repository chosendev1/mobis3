...
$content .= '
<div class="panel-heading">
                              <h3 class="panel-title">BANK ACCOUNT STATEMENT</h3>
                            </div>               
                            <div class="panel-body">';
	...
	if (@mysql_num_rows($st_res) > 0)
	{
		...
		$content .= '<div class="panel panel-default" id="demo">
 		...
 		$content .= '<table class="table" id="table-tools">';
...
function list_saveproduct(){
	...
	if(@ mysql_numrows($sth) == 0){
		...
		$content = "<font color=red>No savings products created yet!</font>";
	...
	$content .= '<div class="panel panel-default" id="demo">
 		...
 		$content .= '<table class="table table-bordered table-hover" id="table-tools">';
...
function list_deposits($mem_no, $name, $product, $from_date,$to_date, $branch_id){
                      ...
                      <div class="form-group">
                         ...
                         <div class="form-group">
                                            ...
                                            <label class="control-label">Date range:</label>
                                               ...
                                               <input type="text" class="form-control" id="from_date" name="from_date" placeholder="From" /><br><br>
                                               <input type="text" class="form-control" id="to_date" name="to_date" placeholder="to" />
                                            </div> </div>                                       
                                             <div class="col-sm-6">     
                                           <label class="control-label">Savings Product:</label>
	...
	$sth2 = mysql_query("select d.receipt_no as receipt_no, d.bank_account as bank_account, d.id as id, d.percent_value as percent_value, d.flat_value as flat_value, m.mem_no as mem_no, m.first_name as first_name, m.last_name as last_name, d.amount as amount, d.date as date, a.account_no as account_no, a.name as name from deposit d join mem_accounts mem on d.memaccount_id=mem.id join  member m on mem.mem_id=m.id join savings_product s on mem.saveproduct_id=s.id join accounts a on s.account_id=a.id where mem.saveproduct_id like '%".$product."%' and (m.mem_no like '%".$dmem_no."%' and (m.first_name like '%".$dname."%' or m.last_name like '%".$dname."%')) and d.date >= '".$from_date."' and d.date <= '".$to_date."' and s.type='free' ".$branch_." order by m.first_name, m.last_name, d.date ");
		if(@mysql_numrows($sth)==0){
		$content .= "<font color=red>No deposits in the selected search criteria. Enter report period</font>";
		$resp->assign("status", "innerHTML", $content);
	//return $resp;
	}
		else{
...
//LIST DISBURSEMENTS
function list_disbursed($cust_name, $cust_no, $account_name, $loan_officer, $from_date,$to_date,$branch_id,$report_format){
	list($from_year,$from_month,$from_mday) = explode('-', $from_date);
        list($to_year,$to_month,$to_mday) = explode('-', $to_date);
	$resp = new xajaxResponse();
	$resp->assign("status", "innerHTML", "");
	...
	$officer_res = mysql_query("select * from employees order by firstName");
...
<br><br>
                                              ...
                                              <select id="report_format" class="form-control"><option value="detail">Detailed<option value="summary">Summarized</select> 
	...
	$content .= "<div class='panel-footer'>                              
                               ...
                               <button type='button' class='btn  btn-primary'  onclick=\"xajax_list_disbursed(getElementById('cust_name').value, getElementById('cust_no').value, getElementById('account_name').value, getElementById('officer_id').value,  getElementById('from_date').value,  getElementById('to_date').value,  getElementById('branch_id').value,  getElementById('report_format').value);\">Search</button>
                            </div>
                        </form>
                        <!--/ Form default layout -->
                    </div>";
	...
	if($from_date=='' && $to_date==''){
		$content .= "<font color=red>Please select the disbursed loans period you would like to list!</font>";
		$resp->assign("status", "innerHTML", $content);
		//return $resp;
	}
	else{
...
$companyd = "unknown".mysql_error();    
              ...
              <div class="panel-body">
               <div class="col-sm-2">
               <img src="'.base_url().'logo/logo.jpg" alt = "logo" style="width:100px; height:100px;padding-top:0px;"/>
               </div>
                 <div class="col-sm-6">
               <h1 class="">'.$compa.' <span class="small">'.$companyb.' Branch</span></h1>
               <h3 class="">'.$reportname.'</h3>
             <br>
               Branch:  <b>'.$branch_didplay.'</b><br>
               Products:  <b>'.$product_didplay.'</b><br>
               Credit Officers: <b>'.$offcer_didplay.'</b><br>
               </div>
                 <div class="col-sm-4">
              Printed on: '.date('Y-m-d h:i:s').'<br>
			...
			/*$content .= '<div class="panelm panel-defaultm" id="demo">
                            <div class="panel-headingb">
                                <h3 class="panel-titlev">Loan Officer:  '.$offcer_didplay.'</h3>
                                <p>Total Amount Disbursed: '.number_format($total__amount['amount'],2).'</p>
                                
                            </div>';*/
 		$content .= '<div class="col-md-12"><table class="borderless" id="table-tools">';
 		$content .= '<thead><th>#</th><th>Loan No.</th><th>Name</th><th>Reciept No.</th><th>Amount Approved</th><th>Amount Disbursed</th><th>Disbursement Date</th><th>Processing Period</th><th>Expiry Date</th><th>Credit Officer</th></thead><tbody>';
			$former_officer = $officer;
		}
...
//expirry
 $expiry = date("Y-m-d H:i:s",strtotime("+".$row['loan_period']."days",strtotime($row['date'])));
		...
		$content .= "<tr><td>".$i."</td><td>".$row['mem_no']."</td><td>".$row['first_name']." ".$row['last_name']."</td><td>".$row['cheque_no']."</td><td>".number_format($row['amount'], 0)."</td><td>".number_format($row['amount'], 0)."</td><td>".$row['date']."</td><td>".$days."</td><td>".$expiry."</td><td>".$officer."</td></tr>";}
		$i++;
		$amt_sub_total += $row['amount']; 
        $totalday += $days;

	}
    if($report_format == "summary" || $report_format == "detail"){
	$content .= "<tfooter><th>Total</th><th></th><th></th><th></th><th>".number_format($amt_sub_total)."</th><th>".number_format($amt_sub_total)."</th><th></th><th>".round($totalday/$i)."</th><th></th><th></th></tfooter></tbody></table></div></div></div>";
	}
}
	$resp->call("createTableJs");
	$resp->assign("display_div", "innerHTML", $content);
...
function risk_ageing($cust_name, $cust_no, $account_name, $loan_officer, $from_date,$to_date,$report_format,$branch_id){
	list($from_year,$from_month,$from_mday) = explode('-', $from_date);
        list($to_year,$to_month,$to_mday) = explode('-', $to_date);
	$resp = new xajaxResponse();
	$resp->assign("status", "innerHTML", "");
	...
	$content = '
                    <div class="col-md-12">
                        ...
                        <form class="panel panel-default" action="">
                            <div class="panel-heading">
                                <h3 class="panel-title">SEARCH FOR PORTFOLIO AT RISK REPORT BY AGEING</h3>
                            </div>               
                            <div class="panel-body">
...
<div class="col-md-6">
                                            <label class="control-label">Reporting Day:</label>
                                            
                                            <input type="hidden" class="form-control" id="from_date" name="from_date" placeholder="From" />
                                                <input type="text" class="form-control" id="to_date" name="to_date" placeholder="to" />
		 ...
		 $content .= "<div class='panel-footer'>                              
                                ...
                                <button type='button' class='btn  btn-primary'  onclick=\"xajax_risk_ageing(getElementById('cust_name').value, getElementById('cust_no').value, getElementById('account_name').value, getElementById('officer_id').value,  getElementById('from_date').value,  getElementById('to_date').value, getElementById('report_format').value, getElementById('branch_id').value);\">Search</button>
                            </div>
                        </form>
                        <!--/ Form default layout -->
                    </div>";
	...
	if($to_date==''){
		$content .= "<div class='alert alert-info'><font color=red>Please select the date for loans you would like to list!</font></div>";
		$resp->assign("status", "innerHTML", $content);
		//return $resp;
	}
	else{
	...
	if(@ mysql_numrows($sth2) == 0){
		$content .= "<div class='alert alert-info'><font color=red>No loans in your search options</font></div>";
		$resp->assign("status", "innerHTML", $content);
		return $resp;
	}
...
//report name
$reportname = "PORTFOLIO AT RISK REPORT BY AGEING"; 
//get company name.
$com = mysql_query("select * from users where userId = '".CAP_Session::get('userId')."'");
$comp = mysql_fetch_array($com);
//var_dump($comp);
...
$companyd = "unknown".mysql_error();    
 		...
 		$content .= '<div class="col-md-12"><table class="table borderless" id="table-tools">
			<tr><th rowspan=2><b>No</b></th><th rowspan=2><b>MemberNo</b></th><th rowspan=2><b>Member Name</b></th><th rowspan=2><b>Amount Disbursed</b></th><th rowspan=2><b>Principal Outstanding</b></th><th rowspan=2><b>Portfolio At Risk</b></th><td colspan=6><b>Portfolio at risk as at '.$to_date.'</b></td></tr>
			<tr><th><b>1-30 Days</b></th><th><b>31-60 Days</b></th><th><b>61-90 Days</b></th><th><b>91-120 Days</b></th><th><b>121-180 Days</b></th><th><b> >180 Days</b></th></tr>';
		...
		////$color=($i % 2==0) ? "lightgrey" : "white";
        if($report_format == "detail"){
		$content .= "<tr><td>".$i."</td><td>".$row['mem_no']."</td><td>".$row['first_name']." ".$row['last_name']."</td><td>".number_format($row['amount'], 2)."</td><td>".number_format($row['balance'], 2)."</td><td>".number_format($row['balance'], 2)."</td><td>".number_format($range1, 2)."</td><td>".number_format($range2, 2)."</td><td>".number_format($range3, 2)."</td><td>".number_format($range4, 2)."</td><td>".number_format($range5, 2)."</td><td>".number_format($range6, 2)."</td></tr>";
    }
		$i++;
        $totalamt += $row['amount'];
    ...
    if($report_format == "summary" || $report_format == "detail"){
	$content .= "<tfooter><tr><th>Total</th><th></th><th></th><th>".number_format($totalamt, 2)."</th><th>".number_format($totalbal, 2)."</th><th>".number_format($totalbal, 2)."</th><th>".number_format($tt1, 2)."</th><th>".number_format($tt2, 2)."</th><th>".number_format($tt3, 2)."</th><th>".number_format($tt4, 2)."</th><th>".number_format($tt5, 2)."</th><th>".number_format($tt6, 2)."</th></tr></tfooter></table></div>";
	}
}
	$resp->call("createTableJs");
	$resp->assign("display_div", "innerHTML", $content);
...
//LIST OUTSTANDING
function list_outstanding($cust_name, $cust_no, $account_name, $loan_officer, $from_date,$to_date,$report_format,$branch_id){
	list($from_year,$from_month,$from_mday) = explode('-', $from_date);
        list($to_year,$to_month,$to_mday) = explode('-', $to_date);
	$resp = new xajaxResponse();
	$calc = new Date_Calc();
	...
	$content .='</select>                                           
                ...
                $content .=' <div class="col-sm-6"><div class="form-group">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <input type="hidden" class="form-control" id="from_date" name="from_date" placeholder="From" />
                                                <input type="text" class="form-control" id="to_date" name="to_date" placeholder="to" /></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
		 ...
		 $content .= "<div class='panel-footer'>                              
                                ...
                                <button type='button' class='btn  btn-primary'  onclick=\"xajax_list_outstanding(getElementById('cust_name').value, getElementById('cust_no').value, getElementById('account_name').value, getElementById('officer_id').value,  getElementById('from_date').value,  getElementById('to_date').value, getElementById('report_format').value, getElementById('branch_id').value);\">Search</button>
                            </div>
                        </form>
                        <!--/ Form default layout -->
                    </div>";
...
$companyd = "unknown".mysql_error();    
              ...
              <div class="panel-body">
               ...
               <img src="'.base_url().'logo/logo.jpg" alt = "logo" style="width:100px; height:100px;padding-top:0px;"/>
                 ...
                 <div class="col-sm-6">
               <h1 class="">'.$compa.' <span class="small">'.ucwords($companyb).'</span></h1>
               <h3 class="">'.$reportname.'</h3>
             <br>
               Branch:  <b>'.$branch_didplay.'</b><br>
               Products:  <b>'.$product_didplay.'</b><br>
 		...
 		/*$content .= '<table class="table borderless" id="table-tools">
 		
			 <thead><th><b>No</b></th><th><b>Member Name</b></th><th><b>MemberNo</b></th><th><b>Date</b></th><th><b>Amount</b></th><th><b>Loan Balance</b></th><th><b>Princ Arrears</b></th><th><b>Int Arrears</b></th><th><b>Total Arrears</b></th><th><b>Penalty</b></th><th><b>Princ Due</b></th><th><b>Int Due</b></th><th><b>Total Amt Due</b></th><th><b>Schedule</b></th><th><b>Payment</b></th></thead><tbody>';*/

             $content .= '<table class="borderless" id="table-tools">
        
             <thead><th><b>No</b></th><th><b>MemberNo</b></th><th><b>Member Name</b></th><th><b>Sex</b></th><th><b>Loan Amount</b></th><th><b>Intrest Amount</b></th><th><b>Disbursment Date</b></th><th><b>Expiry Date</b></th><th><b>Total Paid</b></th><th><b>Loan Balance</b></th><th><b>Amount in arrears</b></th><th><b>Amount not due</b></th></thead><tbody>';
			$former_officer = $officer;
		}
        ...
        //intrest used
        $intrestuded = $row['amount']*($row['intrestrate']/100);
//expire date
         $expiredate = date("Y-m-d H:i:s",strtotime("+".$row['loan_period']."days",strtotime($row['date'])));

		////$color=($i % 2==0) ? "lightgrey" : "white";
         if($report_format == "detail"){
		$content .= "<tr><td>".$i."</td><td>".$row['mem_no']."</td><td>".$row['first_name']." ".$row['last_name']."</td><td>".$row['sex']."</td><td>".number_format($row['amount'], 2)."</td><td>".number_format($intrestuded, 2)."</td><td>".$row['date']."</td><td>".$expiredate."</td><td>".number_format(($row['amount']-$row['balance']), 2)."</td><td>".number_format($row['balance'], 2)."</td><td>".number_format($princ_arrears, 2)."</td><td>".number_format($due_princ, 2)."</td></tr>";
    }
		$i++;
	// GET SUB TOTALS
		$amt_sub_total += $row['amount']; 
	...
	// PRINT SUB TOTALS
    if($report_format == "summary" || $report_format == "detail"){
	$content .= "<tfooter><tr><th>TOTAL</th><th></th><th><b></b></th><th><b></b></th><th><b>".number_format($amt_sub_total, 2)."</b></th><th><b>".number_format($int_sub_total, 2)."</b></th><th><b></b></th><th><b></b></th><th><b>".number_format(($amt_sub_total-$bal_sub_total), 2)."</b></th><th><b>".number_format($bal_sub_total, 2)."</b></th><th><b>".number_format($princ_arrears_sub_total, 2)."</b></th><th><b>".number_format($princ_due_sub_total, 2)."</b></th></tr></tfooter></tbody></table></div>";
	}
}
	$resp->call("createTableJs");
	$resp->assign("display_div", "innerHTML", $content);
...
//ARREARS REPORT
function list_arrears($cust_name, $cust_no, $account_name, $loan_officer, $from_date,$to_date, $status,$report_format,$branch_id){
	list($from_year,$from_month,$from_mday) = explode('-', $from_date);
        list($to_year,$to_month,$to_mday) = explode('-', $to_date);
	$resp = new xajaxResponse();
	$calc = new Date_Calc();
                ...
                $content .=' <div class="col-sm-6"><div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <label class="control-label">Reporting Day:</label>
                                              ...
                                              <input type="hidden" class="form-control" id="from_date" name="from_date" placeholder="From" />
                                                <input type="text" class="form-control" id="to_date" name="to_date" placeholder="to" /><br><br>
                                                <label class="control-label">Report Format:</label>
	...
	if($to_date==''){
		$content .= "<div class='alert alert-info'><font color=red>Please select the report date for the Arrears Report!</font></div>";
		$resp->assign("status", "innerHTML", $content);
		//return $resp;
	}
	else{
...
$companyd = "unknown".mysql_error();    
              ...
              <div class="panel-body">
               ...
               <img src="'.base_url().'logo/logo.jpg" alt = "logo" style="width:100px; height:100px;padding-top:0px;"/>
                 ...
                 <div class="col-sm-6">
               <h1 class="">'.$compa.' <span class="small">'.$companyb.'</span></h1>
               <h3 class="">'.$reportname.'</h3>
             <br>
               Branch:  <b>'.$branch_didplay.'</b><br>
               Products:  <b>'.$product_didplay.'</b><br>
       ...
       //if(strcmp($former_officer, $officer) != 0){
 		$content .= '<table class="borderless" id="table-tools">
			...
			<thead><th><b>#</b></th><th><b>Loan No.</b></th><th><b>Name</b></th><th><b>Loan Amount</b></th><th><b>Interest Amount</b></th><th><b>Principal in Arrears</b></th><th><b>Interest in Arrears</b></th><th><b>Penalty in Arrears</b></th><th><b>Total Arrears</b></th><th><b>Last Repayment Date</b></th></thead><tbody>';
			//$former_officer = $officer;
		//}
            while($row = mysql_fetch_array($sth2)){
		//RESCHEDULE INTEREST FOR DECLINING BALANCE METHOD
		...
		$penalty = ($pen['amount'] == NULL) ? 0.0 : $pen['amount'];
//intrest
     $intmoney = ($row['amount']*($row['intrest']/100));  
     //last repayment
     $las = mysql_query("select date as lastrepayday from payment where loan_id = '".$row['id']."' order by `date` desc limit 1");
     $lastd = mysql_fetch_assoc($las);
		////$color=($i % 2==0) ? "lightgrey" : "white";
     //control format of the report
     if($report_format == "detail"){
	...
	// GET SUB TOTALS
        ...
        $totalout += ($int_arrears+$princ_arrears+$penalty);
	}
	// PRINT SUB TOTALS
 if($report_format == "summary" || $report_format == "detail"){
    //hererererere
...
//REPAYMENT REPORT
function repayment($cust_name, $cust_no, $account_name, $loan_officer, $from_date,$to_date, $status,$report_format,$branch_id){
	list($from_year,$from_month,$from_mday) = explode('-', $from_date);
        list($to_year,$to_month,$to_mday) = explode('-', $to_date);
	$resp = new xajaxResponse();
	$calc = new Date_Calc();
	...
	$content .='</select>                                           
               ...
               <div class="col-sm-6"> <div class="form-group">
                                            ...
                                            <label class="control-label">Loan Status:</label>
                                            <select id="state" class="form-control"><option value="">All<option value="On going">On going<option value="Over Due">Over Due</select>  <br>
                                                <label class="control-label">Report Format:</label>
	...
	if($from_date=='' || $to_date==''){
		$content .= "<font color=red>Please select the period for the Arrears report!</font>";
		$resp->assign("status", "innerHTML", $content);
		//return $resp;
	}
	else{
...
$companyd = "unknown".mysql_error();    
	...
	////$max_page = ceil(mysql_num_rows($sth)/$num_rows);
		...
		/*$content .= '<div class="panel panel-default" id="demo">
 		...
 		$content .= '<table class="borderless" id="table-tools">
			...
			<thead><th><b>#</b></th><th>Name</th><th><b>MemberNo</b></th><th><b>Principle past due start day</b></th><th><b>principle due in period</b></th><th><b>Principle Paid</b></th><th><b>Principle past due start day</b></th><th><b>Intrest Due in period</b></th><th><b>Intrest Paid</b></th><th><b>Penalty past due start day</b></th><th><b>Penalty Due in period</b></th><th><b>Penalty Paid</b></th></th><th><b>Total Amount</b></th></thead><tbody>';
			$former_officer = $officer;
		//}
            while($row = mysql_fetch_array($sth2)){
		//RESCHEDULE INTEREST FOR DECLINING BALANCE METHOD
		...
		////$color=($i % 2==0) ? "lightgrey" : "white";
        if($report_format == "detail"){
		$content .= "<tr><td>".$i."</td><td>".$row['first_name']." ".$row['last_name']."</td><td>".$row['mem_no']."</td><td></td><td></td><td>".number_format($row['amount'], 2)."</td><td>".number_format(($princ_paid_amt + $int_paid_amt), 2)."</td><td>".number_format($princ_arrears, 2)."</td><td>".number_format($int_arrears, 2)."</td><td>".number_format($prepaid, 2)."</td><td>".$repay_rate."</td><td>".$cumm_repay_rate. "</td><td>".number_format($row['balance'], 2)."</td></tr>";
    }
		$i++;
	// GET SUB TOTALS
		$amt_sub_total += $row['amount']; 
	...
	// PRINT SUB TOTALS
    if($report_format == "summary" || $report_format == "detail"){
	$content .= "<tfooter><tr><th></th><th></th><th><b>TOTAL</b></th><th><b></b></th><th><b>".number_format($princ_paid_sub_total, 2)."</b></th><th><b>".number_format($amt_sub_total, 2)."</b></th><th><b>".number_format($total_paid_sub_total, 2)."</b></th><th><b>".number_format($princ_arrears_sub_total, 2)."</b></th><th><b>".number_format($int_arrears_sub_total, 2)."</b></th><th><b>".number_format($prepaid_sub_total, 2)."</b></th><th></th><th><b></b></th><th ><b>".number_format($balance_sub_total, 2)."</b></th></tr></tfooter></tbody></table></div>";
	}
}
	$resp->call("createTableJs");
	$resp->assign("display_div", "innerHTML", $content);
...
{	
	...
	//$resp->append("display_div", "innerHTML", $content);
 		...
 		$content .= '<table class="table table-striped table-bordered" id="table-tools">
		    <thead>
			<th>Date</th><th>Depositor</th><th>Description</th><th>Debit</th><th>Credit</th><th>Account Balance111</th>
		    </thead><tbody>
		    <tr>
	...
	$content = '
                    <div class="col-md-12">
                        ...
                        <form class="panel panel-default" action="">
                            ...
                            <div class="panel-body"><div class="col-sm-6">';
	$content .= '<div class="form-group">
...
// Direct purchases, in-ward transfers and out-ward transfers
	...
	$content .= "<div class='col-md-12'><div id='status'></div>
    <a href='export/shares_ledger?mem_id=".$original_mem_id."&type=".$type."' target=blank()><b>Printable Version</b></a> | <a href='export/shares_ledger?mem_id=".$original_mem_id."&type=".$type."&format=excel' target=blank()><b>Export Excel</b></a>";
	...
	if ($found_shares < 1){
		$content .= "<font color='red'>No Shares Activity Yet Registered For ".$mem['first_name']." ". $mem['last_name']."</font>";
		$resp->assign("status", "innerHTML", $content);
	$resp->assign("display_div", "innerHTML", $content);
	return $resp;
	}
...
function sharing_done($from_date,$to_date,$branch_id){
	...
	if($from_date=='' || $to_date==''){
		...
		$conten .= "<font color=red>Select the period for your report!</font>";
		$resp->assign("status", "innerHTML", $content);
	//return $resp;
	...
	$resp->call("createTableJs");
		...
		$content .= "<font color=red>No dividends shared in the selected period!</font>";
		$resp->assign("status", "innerHTML", $content);
	//return $resp;
	}
	}
	...
	$content = '
                   ...
                   $content .='<div class="col-sm-6"><div class="form-group">
                                            ...
                                            <label class="control-label">Date Range:</label>
                                               ...
                                               <input type="text" class="form-control" id="from_date" placeholder="From" /><br><br>
                                                <input type="text" class="form-control" id="to_date" placeholder="to" />
...
{       $content ="";
	...
	{      
	 $content .="<div class='col-md-12'><a href='export/customers?type=".$type."&mem_no=".$mem_no."&mem_name=".$mem_name."&branch_id=".$branch_id."&stat=".$stat."' target=blank()><b>Printable Version</b></a> | <a href='export/customers?type=".$type."&mem_no=".$mem_no."&mem_name=".$mem_name."&branch_id=".$branch_id."&stat=".$stat."&format=excel' target=blank()><b>Export Excel</b></a>";
	...
	elseif($mem_no == '' && $mem_name==''){
		...
		$content .= "<font color=red>Select a Client.</font>";
		$resp->assign("status", "innerHTML", $content);
	//return $resp;
	}elseif(strtolower($type) == 'all'){
		
		$content .= "<font color=red>No Registered Customers Yet.</font>";
		$resp->assign("status", "innerHTML", $content);
	//return $resp;
	}elseif(strtolower($type) == 'non_members'){
			
		$content .= "<font color=red>No Non Members Registered Yet.</font>";
		$resp->assign("status", "innerHTML", $content);
	//return $resp;
