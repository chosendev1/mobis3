			...
			$branch = mysql_fetch_assoc(mysql_query("select branch_name from branch where branch_no='".$row['branch_id']."'"));
		       ...
		       $content .='
                    <div class="col-md-12">
                        ...
                        <form class="panel panel-default" action="">
                            <div class="panel-heading">
                              </p><table class="table"><tr><td>BRANCH:<font color="#00BFFF">'.$branch['branch_name'].'</font></td>
			<td>MEMBER No:<font color="#00BFFF">'.$row['mem_no'].'</font></td>
				<td>MEMBER NAME:&nbsp;<input type=hidden name="mem_id" value="'.$row['id'].'" id="mem_id"><font color="#00BFFF">'.strtoupper($row['first_name'].' '.$row['last_name']).'</font></td></tr></table></p>
                            </div>               
                            <div class="panel-body">';
			...
			<tr><td><b>Member No:</b></td><td>'.$row['mem_no'].'</td>
				<td><b>Member Name:</b></td><input type=hidden name="mem_account_id" value="'.$mem_account_id.'" id="mem_account_id"><td>'.$row['first_name'].' '.$row['last_name'].'</td></tr></table></p>
                        ...
                        $content.='<div class="form-group">
                                        ...
                                        <div class="col-sm-12">
                                           <table class="table"><tr><td align=right><img src="photos/'.$row['photo_name'].'?dummy='.time().'" width=90 height=90><br>Photo</td><td width=45%>&nbsp;</td><td align=right><img src="signs/'.$row['sign_name'].'?dummy='.time().'" width=90 height=90><br>Signature</td></tr></table>
                                    </div>
                                 </div>';
...
function list_accounts($group_name, $mem_no, $name, $product_id,$branch_id,$num_rows,$stat,$cur_page){
	...
	if($mem_no=='' && $name=='' && $product_type=='')
		...
		if(@ mysql_num_rows($sth2)==0)
			...
			$content .= "<tr class=''><td><b>No</b></td><td><b>Member Name</b></td><td><b>Member No</b></td><td><b>Product</b></td><td><b>Date Opened</b></td><td><b>Balance</b></td><td><b>Status</b></td><td><b>Edit</b></td></tr>";
			$i=$stat+1;
			$group = 'NONE';
...
function add_deposit($mem_no){
	...
	{
		$app = get_instance();
		$mem_no = $app->uri->segment(3);
	}
	//$resp->assign("status", "innerHTML", 'halo');
		...
		while($row = mysql_fetch_array($sth)){
			$content .= "<option value='".$row['mem_no']."'>".$row['first_name']." ".$row['last_name'] ." - ".$row['mem_no'];
                                ...
                                </div></form></div></div><div id="status"></div>';
	 ...
	 if($mem_no=='' || $mem_no=='0'){
		...
		$content .='
                    <div class="col-md-12">
                        ...
                        <form class="panel panel-default" action="">
                            <div class="panel-heading">
                              </p><table class="table"><tr><td>BRANCH:</td><td><font color="#00BFFF">'.$branch['branch_name'].'</font></td>
			<td>MEMBER No:</td><td><font color="#00BFFF">'.$row['mem_no'].'</font></td>
				<td>MEMBER NAME:&nbsp;</td><input type=hidden name="mem_id" value="'.$row['id'].'" id="mem_id"><td><font color="#00BFFF">'.strtoupper($row['first_name'].' '.$row['last_name']).'</font></td></table></p>
                            </div>               
                            <div class="panel-body">';
		...
		if(@mysql_numrows($sth)==0){
			...
			$content .= '<div class="panel panel-default" id="demo">
                            <div class="panel-heading">
                                <p><h5 class="semibold text-primary mt0 mb5">LIST OF DEPOSITS</h5></p>';
                                 if($group_id<>''){
                                 $content .= '<p><h5 class="semibold text-primary mt0 mb5">Group Name:'.$group_name.'</h5></p>
                                 <p><h5 class="semibold text-primary mt0 mb5">Total Savings:'.number_format($group_sum,2).'</h5></p>';
                                 }
                               $content .= '</div>';
 		$content .= '<table class="table table-striped table-bordered" id="table-tools">';
			...
			while($row = @mysql_fetch_array($sth2)){
				...
				$content .= "<tr><td>".$i."</td><td>".$row['first_name']. " ".$row['last_name']."</td><td>".$row['mem_no']."</td><td>".$edit."</td><td>".$row['receipt_no']."</td><td>".$cheque_no."</td><td>".number_format($row['percent_value'], 2)."</td><td>".number_format($row['flat_value'], 2)."</td><td>".$row['date']."</td><td>".$row['account_no']." - ".$row['name']."</td><td>".$bank['account_no']." - ".$bank['name']."</td></tr>";
				$i++;
			}
		}
	}
...
function add_withdrawal($mem_no, $type='cash'){
	...
	$resp->assign("status", "innerHTML", "");
		...
		$resp->assign("status", "innerHTML", "");	
			...
			<td>MEMBER No:</td><td><font color="#00BFFF">'.$row['mem_no'].'</font></td>
				<td>MEMBER NAME:&nbsp;</td><input type=hidden name="mem_id" value="'.$row['id'].'" id="mem_id"><td><font color="#00BFFF">'.strtoupper($row['first_name'].' '.$row['last_name']).'</font></td></tr></table></p>
                            </div>               
                            <div class="panel-body">';
...
function insert_withdrawal($memaccount_id, $mem_id, $amount, $voucher_no, $cheque_no, $bank_account, $date,$branch_id, $trans_to){
	...
	elseif($date < $row['open_date'])  //No withdrawal before the account was opened
		...
		if($row['account_balance'] - $amount < $row['min_balance']){
			$resp->alert("Withdrawal rejected! \n The cash account balance is below the minimum contact manager");
			return $resp;
		}
		...
		elseif($free_amt < $charge_min['min_bal'])
			...
			$resp->alert("Withdrawal registered!");
		}
	}
	return $resp;
}
...
{       
 ...
 $content .= "<div class='panel panel-default' id='demo'>
                            ...
                            <table class='table borderless' id='table-tools'>";
		...
		if(@ mysql_numrows($sth)>0){
			...
			$content .='
			   ...
			   $content .='<div class="form-group">
                                        ...
                                        <div class="col-sm-6">
                                            <label class="control-label">Enter Receiving Account No.:</label><input type=text class="form-control" name="acct_to" id="acct_to" onblur=\'xajax_list_receiver_accts(document.getElementById("acct_to").value); return false;\'></td></tr>';
					$content .= "<tr bgcolor=white><td align=center><div id='receiver1_div' name='receiver1_div'></div></td><td><div id='receiver2_div' name='receiver2_div'></div></td></tr></div></div>";         	
