...
function other_applic($stuff, $loan_type,$entry_mode){
	...
	$content .= '<div class="form-group panel bgcolor-default">
      ...
      while($row = @mysql_fetch_array($sth)){
		...
		</div><input type=hidden id="group_id" value="">
...
if($guarantor1 == $guarantor2){
	...
	if($memid == $guarantor1 || $mem_id == $guarantor2){
	...
	if($mem_id=='' || $marital_status=='' || $residence=='' || $telno=='' || $address=='' || $income1=='' || $income2=='' || $amount=='' || $based_on == '' || $reason=='')
		...
		if($based_on == 'savings'){
			...
			$loanpert = ($amount * $limit['loan_save_percent']) / 100;

			//real possible is
			$x = ($balance*100)/$limit['loan_save_percent'];

			if($balance < $loanpert && $limit['loan_save_percent'] >0){
				$resp->alert("Application rejected! \nThe member can only take ".$x." \n based on his Savings.");
				return $resp;
			}
			...
			$loangur = ($amount * $limit['guarantor_save_percent']) / 100;
$x = ($balance*100)/$limit['guarantor_save_percent'];
			if($balance < $loangur && $limit['guarantor_save_percent'] >0){
				$resp->alert("Application rejected! \nGuarantor 1 can only guarantee ".$x." \n based on his Savings.");
				return $resp;
			}
			...
			$x = ($balance*100)/$limit['guarantor_save_percent'];
			if($balance < $loangur && $limit['guarantor_save_percent'] >0){
				$resp->alert("Application rejected! \nGuarantor 2 can only guarantee ".$x." \nbased on his Savings.");
				return $resp;
			}
			}
			...
			$sharelo = ($amount * $limit['loan_share_percent']) / 100;
			$x = ($balance*100)/$limit['loan_share_percent'];
			if($sharelo > $balance && $limit['loan_share_percent'] >0){
			...
			$sharegu = ($amount * $limit['guarantor_share_percent']) / 100;
			if($sharegu > $balance && $limit['guarantor_share_percent'] >0){
			...
			$x = ($balance*100)/$limit['guarantor_share_percent'];
			$sharegu = ($amount * $limit['guarantor_share_percent']) / 100;
			if($sharegu > $possible && $limit['guarantor_share_percent'] >0){
...
$from_date = sprintf("%04d-%02d-%02d 00:00:00", $from_year, $from_month, $from_mday);
	...
	$content .= '<table class="borderless table-striped" id="table-tools" width=100%>';
 		
 		$content .= '<thead><th>#</th><th>NAME</th><th>MEMBER No.</th><th>AMOUNT</th><th>REASON</th><th>COLLATERAL</th><th>GUARANTORS</th><th>DATE</th><th>LOAN BALANCE</th><th>LOAN Officer</th><th>EDIT COLLATERAL</th><th>APPROVE</th><th>DISBURSE</th></thead><tbody>';
	while($row = mysql_fetch_array($sth2)){
...
list($year,$month,$mday) = explode('-', $date);
	...
	//	$based_on = $prod['based_on'];
		...
		if($based_on == 'savings'){
			...
			//
			$loanper = ($amount*$limit['loan_save_percent'])/100;
			$x = ($balance*100)/$limit['loan_save_percent'];
			if($balance < $loanper && $limit['loan_save_percent'] >0){
			...
			$loanper = ($amount*$limit['loan_share_percent'])/100;
			$x = ($balance*100)/$limit['loan_share_percent'];

			if($loanper > $balance && $limit['loan_share_percent'] >0){
				$resp->alert("Disbursement rejected! \n Customer has insufficient shares for this Loan. \n Only ".$x." is possible");
				return $resp;
			}
			if(!empty($guarantor1) && !empty($guarantor2)){
